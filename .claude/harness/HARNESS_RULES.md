...
## 1.5. Schema v2.1 Extensions

### Ralph Loop Integration

Schema v2.1 adds two optional objects to feature_list.json items for autonomous workflow execution:

#### `ralph_loop` Object

Enables autonomous Ralph Loop execution with iteration tracking and convergence detection.

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `enabled` | boolean | Yes | Enable Ralph Loop for this item |
| `max_iterations` | number | Yes | Safety limit (typically 5-10) |
| `current_iteration` | number | Yes | Progress counter (0 = not started) |
| `convergence_criteria` | object | Yes | Success conditions |
| `convergence_criteria.build_passes` | boolean | Yes | Compilation must succeed |
| `convergence_criteria.tests_pass` | boolean | Yes | Tests must return 0 |
| `convergence_criteria.acceptance_met` | boolean | Yes | Acceptance criteria verified |
...
| Operation | Who Can Do It | When | Rules |
|-----------|---------------|------|-------|
| **SET enabled** | User, INITIALIZER | Enable/disable Ralph Loop | Boolean value |
| **INCREMENT current_iteration** | WORKER | After failed attempt | Use increment_iteration.py script |
| **RESET current_iteration** | User | After human intervention | Set to 0, document in notes |
| **MODIFY max_iterations** | User | Adjust safety limit | Must be > 0 |
| **MODIFY convergence_criteria** | User | Change success conditions | Document reason |

**Iteration Lifecycle:**
1. Item starts with `current_iteration = 0`
2. WORKER attempts implementation
3. On FAILED attempt: increment `current_iteration`
4. On PASSED attempt: status → PASSING (convergence)
5. If `current_iteration >= max_iterations`: status → BLOCKED (requires human review)

**Escape Hatch:**
After human review of BLOCKED item, User can:
- Reset `current_iteration = 0`
- Adjust `max_iterations` if needed
- Add `override_reason` explaining intervention
- Change status back to FAILING to resume

#### `prd_reference` Object

Links feature item to Product Requirements Document for detailed specifications.

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `file` | string | Yes | PRD filename in `.claude/prd/` |
| `last_updated` | string | Yes | ISO8601 timestamp |
| `version` | string | Yes | PRD version (semver) |
...
| Operation | Who Can Do It | When | Rules |
|-----------|---------------|------|-------|
| **ADD prd_reference** | User, INITIALIZER | PRD created | File must exist in `.claude/prd/` |
| **UPDATE file** | User | PRD renamed/replaced | Must update `last_updated` |
| **UPDATE version** | User | PRD content changes | Semver bump, update `last_updated` |
| **REMOVE prd_reference** | User | PRD no longer needed | Document reason in notes |

**Validation:**
- Referenced PRD file MUST exist in `.claude/prd/` (WARNING if missing)
- PRD file MUST validate against `.claude/prd/schema.json` (WARNING if invalid)
- Cross-reference check in `scripts/verify.sh`

### Backward Compatibility

Schema v2.1 is **fully backward compatible** with v2.0:
- Existing items without `ralph_loop` or `prd_reference` remain valid
- Validation ignores unknown fields (INFO level)
- Scripts handle missing fields gracefully with defaults
