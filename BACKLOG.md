...
- **Director autonomous owner-clobber on effect switch** (introduced as side-effect of authority-inversion fix landing 2026-05-18, commits `463220b0`/`da24ed1a`). `SynqMatrix.cpp:976` (queueDirectorSwitch) and `:1009` (`notifySwitchApplied`) unconditionally write `m_owner = Director`, overwriting `Manual` whenever a switch event fires. Manifests as: any user action that mutates Zone Composer state (e.g. `/api/v1/zones/layout`) successfully sets owner=Manual at the handler, but the next Director-side notification clobbers it back to Director. The `/api/v1/zones/enabled` path is unaffected because no Director switches fire while Zone Mode is ON. **Scope:** make both writes conditional — `if (current_owner != Manual && current_owner != Show) setOwner(Director)`. Closure: planned via SSA-C (Task #6) — Universal Authority Inversion + Director Startup Preference.
- **REST per-zone regex POST routes return 404** — `POST /api/v1/zones/{1-3}/effect|brightness|speed|palette|blend|enabled` registered via `registry.onPostRegex` at `V1ApiRoutes.cpp:1092-1146` are not matching incoming requests. Pre-existing (predates the 2026-05-18 authority-inversion patch). The WS path `zone.setEffect/setPalette/setBlend/...` is the canonical client path used by iOS, Tab5, and the fallback HTML controller, so this gap does not block production control surfaces — but the REST routes should still work as documented. Investigation needed in `HttpRouteRegistry::onPostRegex` matcher.
- **`[WebServer] Low-heap shedding — DIAGNOSED 2026-05-18.** Witchhunt complete: 5-SSA forensic audit + 2nd/3rd external opinions converged. Root cause is **structural internal-SRAM deficit** — application heap is ~28 KB at idle vs the 30 KB shed threshold; system is configured for ~400 KB of demand on 320 KB hardware. **Hex-ID architecture is NOT the cause** (PROGMEM `const char*` correctly pointer-aliased). Highest-leverage move per both external reviewers: PSRAM-allocate `AsyncWebSocketSharedBuffer` (+24-40 KB *retained* internal-DRAM relief at 4-8 clients). Reconciled 3-tier plan + diagnostic firmware on branch `ssa-w4-diagnostic-build` (`fc7ae2bf`) + soak protocol bundled in branch tree. Full context: `~/.claude/projects/<slug>/memory/project_k1_heap_pressure_witchhunt_2026_05_18.md`. Three open Qs gate Phase 1 dispatch: (1) does `uxTaskGetStackHighWaterMark` telemetry exist on K1 V2? (2) what ArduinoJson minor version (7.3+ requires `JsonString(ptr,true)` wrapping)? (3) nm/map output — are `PATTERN_METADATA` + name literals in `.flash.rodata` or `.dram0`? Captain's Phase 3 reframe: gate `ledTransport.*` behind a dev-only `stats.telemetry` topic — was designed as autonomous-agent backend dev-signal AND iOS preview novelty; production status broadcast drops the 12-field nested object. Witchhunt artefacts at `docs/research/heap-pressure-witchhunt-2026-05-18/`.
...
- **`/synqmatrix/engage` owner transient resets to `none`** — hardware validation 2026-05-18 of commits `06cc6e76` + `7444599d` showed `POST /api/v1/synqmatrix/engage` correctly returns the SynqMatrixStatus payload with `owner: "director"` inline, but a subsequent `/device/status` query ~1 s later reports `owner: "none"` while `mode` remains `director`. Likely cause: SynqMatrix's suppression machinery writes `setSuppressed(reason, None)` (e.g. for `boot_grace`, `disabled`, `low_confidence`) immediately after engage, and the Block 1 gating from SSA-C v2 (commit `06cc6e76`) only gates `Owner::Director` writes — `None` writes pass through. Net effect: Director is engaged (mode=director, ticking) but ownership reads as None, which is semantically odd. Not a regression vs prior behaviour — engage simply wasn't reachable before. Workaround: existing user-action paths still correctly assert Manual and Manual still preempts Director. Closure: review the suppression-reason → owner mapping in `SynqMatrix::setSuppressed`, decide whether suppressed-Director should report `Director` (with the reason explaining why it's idle) or `None`. Likely the right semantic is: Director engaged → owner stays `Director` even when suppressed for confidence/grace/etc.; only an explicit `release` writes owner=None.
...
- **Engineering scope (4 tasks):**
  ...
  2. **PARTIAL (decision brief ready): Decide band-restructuring approach.** Three viable options: (a) drop the highest band (typically least musically informative), (b) merge two adjacent bands (sub-bass + bass, or upper-mid + treble), (c) re-partition to 3 logarithmic-spectrum buckets that better match the user-facing visual zone semantics. Captain requested the need for this process be explained before choosing. Decision-enabling brief: `firmware-v3/docs/research/f6_controlbus_band_restructuring_decision_brief_2026-05-16.md`. Implementation remains blocked until Captain selects the approach.
