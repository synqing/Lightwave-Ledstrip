...
- **Revisit trigger:** Multiple-router pure-STA validation, F-5 Task 2 mode-preference implementation, or an ESP-IDF 5.x upgrade that resolves the concurrent AP+STA bug at the driver level (current K1 is pinned at IDF 4.4.7; upgrade is blocked by an I2C bug + API rewrites per `firmware_build_envs.md`).
