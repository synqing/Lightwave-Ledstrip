...
### Added
- **firmware-v3/test (EdgeMixer→K1 port oracle):** Native golden-master parity harness — Step 0 of `docs/research/edgemixer_k1_port_plan_2026-07-08.md`. Freezes 810 golden input→output vectors (6 colour-harmony modes × 5 spreads × 27 probes) generated by driving the **unmodified** `EdgeMixer::process()` at strength 255 / uniform / static (the scale8-independent direct-assign path), and proves a Q15.16 fixed-point reference port (the code that transplants to K1's SQ15x16) reproduces them within **±1 LSB**. Includes a fault-evidence case (perturbing one matrix coefficient breaks parity → RED, restored → GREEN) and near-black passthrough coverage. 4/4 Unity cases pass (independently re-run). Additive only; `src/` untouched; not committed by default. Run: `pio test -e native_test_edgemixer_parity`. Refs: `firmware-v3/test/test_edgemixer_parity/`, `firmware-v3/platformio.ini` (`[env:native_test_edgemixer_parity]`), `BACKLOG.md` (EdgeMixer defects + WB-3).
