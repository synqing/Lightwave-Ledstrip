...
### Added
- **tools:** `firmware-v3/tools/contract-regen/` — REST + WS drift detector for `docs/protocol/k1-{rest,ws}-contract.yaml`. Two Python tools (stdlib + PyYAML) scanning `V1ApiRoutes.cpp` and `Ws*Commands.cpp` to surface drift. Modes: drift report (default), `--strict` (CI exit codes), `--update-skeleton` (FIXME-placeholder YAML). 21/21 tests passing. Real-tree drift surfaced: REST 26 (25 firmware-only + 1 contract-only POST /update), WS 3 (`merge.submit`, `vrms.subscribe`, `vrms.unsubscribe`). Implements F-1's "regenerate from firmware route registry on demand" decision. PR #13.
- **firmware (network):** Heap-shed fragmentation latch fix — `WebServer.cpp:575` `heapRecovered` gate gains a secondary check on largest contiguous free block. Pre-fix: total-free-heap-only gate could not release the latch when heap was fragmented (largest stuck at ~3-4 KB while total recovered to 10-12 KB); under iOS reconnect storm the latch became permanent. Post-fix: latch releases when largest > `LW_INTERNAL_HEAP_LARGEST_BLOCK_RECOVERY_BYTES` (default 8 KB) AND total free above shed floor. New build-flag-overrideable macro added in `WebServer.h`. Investigation traced root cause to `LedStreamBroadcaster` per-client message queue × 8 LED frame slots × 960 B = 7680 B chunk pinned for client lifetime. PR #14.
- **ios:** Phase 2 firmware parity — five user-facing surfaces shipped via 5 parallel sandboxed SSAs:
  - `EffectParameterSheet` runtime parameter UI with type-appropriate controls (FLOAT→Slider, INT→Stepper, BOOL→Toggle, ENUM→Picker) per F-4. Per-parameter 150 ms debounce, GET/POST `/api/v1/effects/parameters` wiring. 12 tests.
  - STM (250 B binary, magic 0xFD, 30 Hz) + VRMS (JSON envelope, 10 Hz) WS subscriptions + `STMSpectralCard` + `VRMSPerceptualCard`. 7 tests.
  - `PresetsViewModel` + `PresetsView` fully implemented (was stub) — REST list + WS save/load/delete + broadcast-driven refresh + `PresetWebSocketCommanding` test seam. 11 tests.
  - `ShowViewModel` + `ShowsView` with Now Playing + transport (was stub) — REST list + WS show.{play,pause,resume,stop,seek,status}. 12 tests.
  - Effect picker `showExperimental` power-user toggle + EXPT badge + Optional `isExperimental` DTO field decode. 5 tests.
  - 47 new Phase 2 tests + 68 Phase 1 baseline = 115/115 green on `xcodebuild test`. PR #12.
- **docs:** `docs/superpowers/ios-firmware-parity-phase-2.md` — Phase 2 plan capturing F-1..F-4 Captain-delegated decisions (resolved 2026-05-01) and the 5-SSA execution model.
- **docs:** `docs/superpowers/ios-firmware-parity-phase-3-scoping.md` — Phase 3 scoping doc synthesising 4 parallel SSA findings (show upload/delete, preset broadcast wiring, ENUM case-name display, contract YAML regenerator) into a Captain decision menu (D-1..D-5).
- **docs:** `BACKLOG.md` § Critical — Upstream Calibration Debt — F-1..F-4 marked DECIDED 2026-05-01 with rationale: F-1 firmware = source-of-truth; F-2 resolved by `isExperimental` flag; F-3 keep legacy paths; F-4 end-user runtime-parameter surface.
