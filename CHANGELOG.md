...
### Added
- **firmware-v3/docs (reference):** K1 V2 IDF 4.4.7 API addendum to `ESP32-S3-Canonical-Reference-Skills.md`. Restates I2S / RMT / GDMA / cache APIs using legacy 4.4.7 headers — all signatures verified verbatim against `~/.platformio/packages/framework-espidf@3.40407.240606` (ESP_IDF_VERSION 4.4.7). Audio-pipeline audit correction: K1 V2's actual I2S init diverges from the canonical SPH0645 textbook config in three coupled ways (`STAND_MSB` not Philips, `RIGHT_LEFT` not `ONLY_LEFT`, `>>10` not `>>14`) — validated by bugfix #23164 (2026-01-30 "I2S Audio Capture Legacy Driver Bit Shift Correction") and re-verified from `src/audio/AudioCapture.cpp:695–748` + `src/config/audio_config.h:44–146`. Includes verified-from-source K1 V2 I2S init block with mic-type-aware (`MicType::SPH0645` / `IM69D130`) post-`i2s_set_pin` register tweaks (`I2S_RX_LEFT_ALIGN`, `I2S_RX_TIMING_REG BIT(9)`, `I2S_RX_MSB_SHIFT`), sample-extraction pattern (`CHANNEL_OFFSET` + `BIT_SHIFT` compile-time switch on `MicType`), divergence map vs canonical, and "what NOT to copy" list. Heap-shed measurement section flagged as load-bearing concrete next step for the active heap-pressure witchhunt — recommends shed logic gate on `heap_caps_get_info(MALLOC_CAP_DMA \| MALLOC_CAP_INTERNAL).largest_free_block` (SRAM1 D-view specifically), not `MALLOC_CAP_INTERNAL` aggregate which includes idle RTC FAST. Refs: `firmware-v3/docs/specs/ESP32-S3-K1-IDF4.4.7-API-Addendum.md`.
- **repo (mcp):** Wired Espressif Documentation MCP (`https://mcp.espressif.com/docs`) into `.mcp.json` as `espressif-docs`. OAuth 2.1 + Dynamic Client Registration; first invocation triggers browser auth (scope `read:user`, PKCE S256). Rate limit 40 req/h / 200 req/day. Verified 2026-05-18 via 3 probes against the live MCP: WiFi (`esp_wifi_*`) + mbedTLS APIs are FULL applicability to IDF 4.4.7 (stable APIs across 4.4.7 → 5.x); RMT returns 5.x driver only (RMT4 lives in migration guide → use local headers / clangd for that subsystem). mbedTLS probe returned verbatim "40–50 KB heap per TLS handshake" figures that directly cite-back into the heap-pressure witchhunt and the dev-only Espressif-OpenAI-component rule. Refs: `.mcp.json`.
- **firmware-v3/docs:** Landing-page Liquid Light source direction now pivots the hero-motion capture brief from Waveform Hybrid to the Holographic family: controlled `0x0201` first, constrained `0x1000` only after palette narrowing. Refs: `firmware-v3/docs/research/lp_hero_liquid_light_holographic_direction_2026-05-08.md`.
- **firmware-v3/tools:** Liquid Light palette sequence runner drives the existing `0x0201 LGP Holographic` serial control path for repeatable K1v2 LP hero source auditions, and records the first `/dev/cu.usbmodem2101` K1v2 execution evidence. Refs: `firmware-v3/tools/liquid_light_palette_sequence.py`, `firmware-v3/docs/research/lp_hero_liquid_light_palette_run_2026-05-08.md`.
- **repo:** Work Blocking protocol for critical discoveries found during another mission. Adds a versioned governance policy, links it from `CLAUDE.md` and `AGENTS.md`, and records the first two Work Blocks: systemic naming/metric accountability and FastLED/RMT transport visibility. Refs: `instructions/work-blocking-protocol-v1.md`, `BACKLOG.md` § Critical — Work Blocks.
- **firmware-v3:** Synergy-Topology Phase 3 Move 3.3 InterStripPhaseDelay substrate. Adds a paired dual-strip delay-line helper over PSRAM frame rings so future `DUAL_CHANNEL` effects can sample delayed strip A/B frames without render-path allocation. Adds scoped native test coverage and includes it in `scripts/native_harness_matrix.py`. Refs: `BACKLOG.md` Synergy-Topology Programme Phase 3.
- **firmware-v3:** Synergy-Topology Phase 3 Move 3.1 Reflective Twin enforcement. Adds `ReflectiveTwinPolicy` so direct dual-strip output is allowed only when effect metadata declares `EffectRoleFlags::DUAL_CHANNEL`; default effects stay on the mirrored unified path. Adds scoped native test coverage and includes it in `scripts/native_harness_matrix.py`. Refs: `BACKLOG.md` Synergy-Topology Programme Phase 3.
- **firmware (zones) — D-1 Spike 1 prototype (2026-05-02):** Per-zone effect instance pool prototype landed; **hardware validation pending; D-3 remains blocked**. Adds `firmware-v3/src/effects/zones/ZoneEffectPool.{h,cpp}` — fixed-capacity slot table (`kMaxSlots = MAX_ZONES * 3 = 9`) keyed by `(effectId, zoneSlot)`. On miss, asks an injected `IZoneEffectSource` adapter (`RendererActor` in firmware; mock in tests) for a registered `EffectFactoryFn` and constructs a fresh per-zone instance; on hit, returns the cached instance. `RendererActor` grew `IEffectRegistry` factory field, `registerEffectFactory()` / `getEffectFactory()` APIs, and now implements `IZoneEffectSource`. Live render-path swap from singleton lookup to pool `acquire()` is **GATED behind `K1_ZONE_INSTANCE_POOL_ENABLED` (default `0`)** — pool is constructed in every build, but `renderZone()` / `setZoneEffect()` / `loadPreset()` retain the verbatim 2026-05-02 D-1 partial-fix path until Captain flips the gate after hardware validation. Memory: ~157 B static DRAM (slot table + active-effect cache), ~720 B PSRAM per K1 Bloom instance worst case (3 unique active ≈ 2.2 KB PSRAM, ≪ 0.1 % of 8 MB Octal headroom). Render-path lookup: O(1) array dereference (gate-on) vs O(N=162) linear registry scan (gate-off) — strictly faster on the hot loop. Builds: gate-off RAM 43.6 % / Flash 33.4 % (production path); gate-on adds +436 B Flash (per SSA sandbox measurement). Native test `test_zone_effect_isolation` — 7/7 PASSED (1.145 s) — covers distinct-instances-per-pair (the D-1 invariant), single `init()` per pair, reacquire idempotency, factory-absent singleton fallback, `clear()` heap safety, invalid-slot rejection, peek-no-alloc. ADR § D-1 status remains "spike in progress / prototype" — NOT marked implemented. Refs: `docs/adr/zone-composer-architecture-decisions.md` § D-1 Spike 1 implementation notes, `firmware-v3/test/test_zone_effect_isolation/README.md`.

  [deferred hardware validation: pool isolation hardware test pending; gate stays default-off until Captain flips post-validation]
...
### Fixed
- **firmware-v3 (serial):** Lone keystroke hotkeys (`Q`, `x`, space, `+`/`-`, etc.) no longer stranded when USB-CDC delivers them alongside trailing bytes (CR/LF echo, framing, subsequent keys). The `shouldProcess` gate introduced in `1eb6155e` (2026-05-03) and re-shipped in `adacee3d` (2026-05-06) only set its flag when `Serial.available() == 0` at byte-read time; with trailing bytes pending, lone hotkeys were buffered indefinitely waiting for a newline that never arrives. Fix factors the hotkey allowlist into `SerialCLI::isImmediateHotkeyChar()` (single source of truth for both the in-loop guard and a new end-of-tick safety net) and dispatches the buffered character when the buffer is length-1 and allowlist-matched. Multi-char commands whose first letter is also a hotkey (`adbg N`, `ae`, `eM`, `b...`, `v...`, `t...`) and JSON line buffering preserved unchanged. Validated on K1v2 (`b4:3a:45:a5:87:f8`): lone `Q` dispatches SynqMatrix status; lone `x` dispatches bands dump; `adbg` shows level (does NOT split into `a` + `dbg`); `ae` shows auto-exposure; `cc` still works; 7/7 behavioural checks PASS. Refs: `firmware-v3/src/serial/SerialCLI.{h,cpp}`.
- **firmware-v3/tools:** Liquid Light palette runner now uses Captain's selected LP hero palette sequence: Red Magenta Yellow, Autumn 19, Fire, Emerald Dragon, Vintage 57, GR64 Hult, Vintage 01. K1v2 run evidence recorded with clean post-run show/timing packet. Refs: `firmware-v3/tools/liquid_light_palette_sequence.py`, `firmware-v3/docs/research/lp_hero_liquid_light_captain_palette_2026-05-09.md`.
- **firmware-v3/tools:** Liquid Light palette runner now supersedes the rejected soft palette sequence, forces a real Holographic re-init via `0x0200 -> 0x0201`, and uses direct SerialJSON control by default so capture auditions do not walk through unrelated intermediate palettes. Refs: `firmware-v3/tools/liquid_light_palette_sequence.py`, `firmware-v3/docs/research/lp_hero_liquid_light_palette_recut_2026-05-09.md`.
- **firmware-v3/docs:** Waveform characterisation docs now reflect that `vp stack` is a partially implemented read-only serial surface, not only a future draft, and record the post-Work-Block K1v2 `0x1313`/`0x1302` baseline plus clangd-backed Waveform source mechanism anchors. No firmware behaviour changed. Refs: `firmware-v3/docs/research/k1_visual_characterisation_database.md`, `firmware-v3/docs/research/k1_waveform_hybrid_serial_evidence_2026-05-07.md`, `firmware-v3/docs/debugging/VP_STACK_INTROSPECTION_COMMAND_SPEC.md`.
- **firmware-v3:** Harden native Unity harness routing by retiring the stale broad native aggregate, adding a scoped native harness matrix, and guarding CI/docs against stale native-test routes. Refs: `docs/superpowers/plans/2026-05-05-native-harness-hardening.md`.
- **firmware-v3:** Phase 5 PVF/BPS candidate repair checkpoint: PVF no longer updates chroma structure from sustained non-onset hops or raw RMS beds, and BPS no longer paints an always-alive RMS/gHue bed before event sprites. Native regressions cover sustained non-event silence; hardware visual status remains degraded pending the next K1v2 PVF/BPS pass. Refs: `BACKLOG.md` Synergy-Topology Programme Phase 5.
- **tools/docs:** claude-mem search routing documentation now distinguishes memory search from Smart Explore code navigation, treats `smart_search` / `smart_outline` `Transport closed` as client MCP transport degradation rather than worker outage, and directs agents to worker `/api/search`, SQLite FTS, `$RECALL_CLI`, `rg`, or clangd fallback paths. Refs: `docs/tooling/claude-mem-usage-optimisation-2026-05-02.md`, `docs/WORKFLOW_ROUTING.md`.
- **tools/docs:** Codex clangd semantic-routing support now has repo documentation, a local `firmware-v3/.clangd` compatibility file, and a scoped reset helper for poisoned clangd MCP child transports. Refs: `CLAUDE.md`, `AGENTS.md`, `docs/WORKFLOW_ROUTING.md`.
- **tools:** claude-mem Smart Explore C/C++ outline repair. Local `claude-mem` 12.4.9 runtime now has a Bun-compatible `tree-sitter` prebuild path and patched Smart Explore query routing for JavaScript vs TypeScript and C/C++; direct `tree-sitter query` verification extracts symbols from `AudioEffectMapping.{h,cpp}`, `AudioHandlers.cpp`, and `test_audio_mapping_registry.cpp`. Existing Codex MCP transport may still require a fresh session attach after stale MCP stdio processes are terminated. Refs: `docs/tooling/claude-mem-usage-optimisation-2026-05-02.md`.
...
### Changed
- **ios (zones) — Wire-format migration (2026-05-02):** iOS LightwaveOS app migrated to **1-indexed everywhere** internally — diverges from Tab5/Dashboard (which kept 0-indexed internal + boundary translation). Rationale per SSA: iOS lacked a centralised wire boundary; pre-migration code was already half-converted (display labels did `+1`, storage was 0-indexed) and the split was fragile. `ZoneConfig.id` and `ZoneSegment.zoneId` now carry the wire 1..3 value directly. Outbound: `RESTClient.setZone{Effect,Brightness,Speed,Palette,BlendMode,Layout}` interpolate `zoneId` verbatim into `/api/v1/zones/{zoneId}/...`; new `validateWireZoneId(_:)` static helper rejects 0 and out-of-range with new `APIClientError.invalidZoneId(Int)` BEFORE network round-trip. Inbound: `ZoneViewModel.handleZoneUpdate` has three log-and-drop guards (zones[].id == 0, segments[].zoneId == 0, targeted zoneId == 0 broadcast) — never crashes on bad firmware data. UI labels de-skewed: redundant `+ 1` removed from accessibility labels and selector chips so user-facing strings match wire format directly. `boundaryIndexForZone` (boundary slot model, NOT zoneId) updated to map `segment.zoneId == 1 → b0`, `== 2 → b1`. `Color.zoneColor()` keeps its 0-indexed array semantic; callers translate with `max(0, zoneId - 1)`. Build pass: `xcodebuild -scheme LightwaveOS -destination 'platform=iOS Simulator,name=iPhone 16,OS=18.6'` BUILD SUCCEEDED. Tests pass: 155/155 (9 new in `LightwaveOSTests/ZoneIdWireFormatTests.swift` covering REST/WS guard paths and URL interpolation). Refs: firmware commit `d53092ad`.

  [deferred hardware validation: REST/WS round-trip with K1 hardware pending next window]
- **tab5 (network) — Wire-format migration (2026-05-02):** WS zone commands send 1-indexed `zoneId` (1..3) outbound and decode 1-indexed inbound, matching K1 firmware B2 contract. Internal C++ storage stays 0-indexed (`ZoneState[3]`, `ZoneSegment.zoneId`, `ParameterId::ZoneN`); translation via new `toWireZoneId()` helper at the WS boundary only. Outbound: all 5 send methods (`sendZoneEffect/Brightness/Speed/Palette/Blend`) + `sendZonesSetLayout` (per-segment) + `processSendQueue()` (queued zone parameters). Inbound: `WsMessageRouter::handleZoneStatus` and `handleZonesList` (segments + zones[] + sidebar pass) reject wire 0 (RESERVED) and out-of-range values before mapping to internal storage. **Bonus safety win:** range guards in send methods hoisted ABOVE `paramIndex = ZONE1_EFFECT + zoneId*2` calculation (previously index computed first → undefined behaviour on `zoneId = 255`). No `src/ui/` LVGL touches; no NVS migration (persisted layouts retain 0-indexed internal). Build pass on `pio run -e tab5` (ESP32-P4, RAM 3.3%, Flash 33.3%). Refs: K1 firmware commit `d53092ad`, `tab5-encoder/CHANGELOG.md`.

  [deferred hardware validation: WS round-trip with K1 hardware pending next window]
- **dashboard (zones) — Wire-format migration (2026-05-02):** Lightwave Dashboard v2 services send `zoneId` 1-indexed (1..3) outbound and parse 1-indexed inbound, matching K1 firmware B2. `state/v2.tsx` `zones.changed` handler refactored from array-index lookup to id-field match (`reduceZonesChanged`) — robust for non-contiguous wire IDs. `ZoneEditor.tsx` `PRESET_SEGMENTS` migrated to 1-indexed; max zones clamped to 3 (Quad Active 4-zone preset removed → Triple Wide 3-zone). `validateLayout()` checks `seg.zoneId === i + 1`; error messages reference `seg.zoneId` directly. `types.ts`: `V2ErrorCode.INVALID_VALUE` added; JSDoc documents 1-indexed wire format on `V2Zone.id` / `V2ZoneSegment.zoneId`. Build pass: `tsc -b && vite build` (294.54 kB app.js, 1709 modules). Zone-related tests 17/17. **Scope-creep additions** (added by SSA to unblock pre-existing untracked v2 test specs): `V2WsClient` accepts options + sends `auth{type, apiKey}` on open; `effectsList`/`palettesList` accept optional `page?` param; pure-function exports for state reducers (`extractWsParameters`/`mergeParameters`/`extractZonesList`/`reduceZonesChanged`); `V2Parameters.mood` optional + `hue` field. Refs: firmware commit `d53092ad`, `docs/protocol/k1-{rest,ws}-contract.yaml`.
