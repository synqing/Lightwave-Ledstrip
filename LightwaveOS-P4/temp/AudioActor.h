 ...
 * Runs on Core 0 at priority 4 (below Renderer at 5).
 * 
 * SELF-CLOCKED MODE (tickInterval=0):
 * AudioActor runs in self-clocked mode where onTick() blocks on I2S read.
 * This allows the I2S DMA to be the timing source (125 Hz = 8ms hops),
 * bypassing the 100 Hz FreeRTOS tick limitation.
 */
inline actors::ActorConfig Audio() {
    return actors::ActorConfig(
        ...
        16,                                         // queueSize
        0                                           // tickInterval=0: self-clocked via I2S blocking read
    );
}
