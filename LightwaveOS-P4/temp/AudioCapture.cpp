...
bool AudioCapture::configureI2S()
{
    ...
    if (s_es8311 == nullptr) {
        s_es8311 = es8311_create(I2C_NUM_0, es8311_addr);
    }
    if (s_es8311 == nullptr) {
    ...
    err = es8311_sample_frequency_config(s_es8311, SAMPLE_RATE * I2S_MCLK_MULTIPLE, SAMPLE_RATE);
    if (err != ESP_OK) {
        LW_LOGE("es8311_sample_frequency_config failed: %s", esp_err_to_name(err));
        return false;
    }
    
    err = es8311_microphone_config(s_es8311, false);
    if (err != ESP_OK) {
