...
void IRAM_ATTR process_GDFT() {
  float MOOD_VAL = 0.05;  // Default value
  uint8_t LIGHTSHOW_MODE = LIGHT_MODE_GDFT;  // Default value
  
  // SINGLE-CORE OPTIMIZATION: Direct access (both threads on Core 0)
  MOOD_VAL = CONFIG.MOOD;
  LIGHTSHOW_MODE = CONFIG.LIGHTSHOW_MODE;
  
  if (LIGHTSHOW_MODE == LIGHT_MODE_BLOOM) {
    MOOD_VAL = 1.0;
  }
