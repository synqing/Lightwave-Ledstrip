...
// Color mode definitions for palette system
enum ColorMode {
  COLOR_MODE_HSV = 0,      // Original HSV mode (default)
  COLOR_MODE_PALETTE = 1,  // Palette-based colors
  COLOR_MODE_HYBRID = 2    // Blend between HSV and palette
};

// Palette system functions
CRGB16 get_chromatic_palette_color(uint8_t note_index, SQ15x16 value);
CRGB16 get_mode_color(SQ15x16 hue, SQ15x16 saturation, SQ15x16 value);
CRGB16 get_palette_color_fixed(uint8_t index, uint8_t brightness);
CRGB16 interpolate_palette_hue(SQ15x16 hue);
void load_palette_safe(uint8_t index);
void update_palette_transition();
CRGB16 blend_colors(CRGB16 color1, CRGB16 color2, uint8_t blend_amount);
...
void clip_led_values(CRGB16* buffer) { // Modified to accept buffer pointer
...
extern uint8_t gCurrentPaletteIndex;

// Palette transition state
uint8_t gPaletteTransitionProgress = 255;  // 0-255, where 255 = complete
bool gPaletteTransitioning = false;
uint32_t gPaletteTransitionStartTime = 0;
const uint32_t PALETTE_TRANSITION_MS = 1000;  // 1 second transitions

// Convert CRGB (8-bit) to CRGB16 (fixed point)
inline CRGB16 CRGB_to_CRGB16(const CRGB& color) {
    CRGB16 result;
    result.r = SQ15x16(color.r) / SQ15x16(255.0);
    result.g = SQ15x16(color.g) / SQ15x16(255.0);
    result.b = SQ15x16(color.b) / SQ15x16(255.0);
    return result;
}

// Get color from current palette at given position (0-255), brightness 0-255
// Update palette transition progress
void update_palette_transition() {
    if (!gPaletteTransitioning) return;
    
    uint32_t elapsed = millis() - gPaletteTransitionStartTime;
    if (elapsed >= PALETTE_TRANSITION_MS) {
        // Transition complete
        gCurrentPalette = gTargetPalette;
        gPaletteTransitioning = false;
        gPaletteTransitionProgress = 255;
    } else {
        // Calculate progress (0-255)
        gPaletteTransitionProgress = (elapsed * 255) / PALETTE_TRANSITION_MS;
    }
}

CRGB16 get_palette_color_fixed(uint8_t index, uint8_t brightness) {
    // Get color directly from current palette (no transitions for now)
    CRGB rgb = ColorFromPalette(gCurrentPalette, index, brightness, LINEARBLEND);
    
    // DEBUG: Print palette color values for troubleshooting
    if (index == 128 && millis() % 1000 < 50) {  // Only print occasionally
        USBSerial.print("PALETTE DEBUG: index=");
        USBSerial.print(index);
        USBSerial.print(" brightness=");
        USBSerial.print(brightness);
        USBSerial.print(" rgb=(");
        USBSerial.print(rgb.r);
        USBSerial.print(",");
        USBSerial.print(rgb.g);
        USBSerial.print(",");
        USBSerial.print(rgb.b);
        USBSerial.println(")");
    }
    
    return CRGB_to_CRGB16(rgb);
}

// Interpolate palette hue for chromatic or gradient modes
CRGB16 interpolate_palette_hue(SQ15x16 hue) {
    // Map hue (0-1 fixed point) to palette index (0-255)
    uint8_t palette_index = (hue * SQ15x16(255)).getInteger();
    
    // For now, just use direct palette mapping
    // TODO: Add chromatic note mapping support later
    return get_palette_color_fixed(palette_index, 255);
}

// Load a palette from PROGMEM into RAM
void load_palette_safe(uint8_t index) {
    if (index >= gGradientPaletteCount) {
        index = 0;  // Fallback to first palette
    }
    
    // If we're already at this palette, do nothing
    if (index == gCurrentPaletteIndex) {
        return;
    }
    
    // FIXED: Load palette using direct assignment (FastLED standard method)
    gCurrentPalette = gGradientPalettes[index];
    gCurrentPaletteIndex = index;
...
// Blend two CRGB16 colors
CRGB16 blend_colors(CRGB16 color1, CRGB16 color2, uint8_t blend_amount) {
    SQ15x16 blend = SQ15x16(blend_amount) / SQ15x16(255.0);
    SQ15x16 inv_blend = SQ15x16(1.0) - blend;
    
    CRGB16 result;
    result.r = color1.r * inv_blend + color2.r * blend;
    result.g = color1.g * inv_blend + color2.g * blend;
    result.b = color1.b * inv_blend + color2.b * blend;
    
    return result;
}

// Chromatic note to palette mapping
// Maps the 12 chromatic notes to evenly spaced positions in the palette
CRGB16 get_chromatic_palette_color(uint8_t note_index, SQ15x16 value) {
    // Map 12 notes across the palette (0-11 -> 0.0-1.0)
    SQ15x16 palette_position = SQ15x16(note_index) / SQ15x16(12.0);
    
    // Get the base color from palette
    CRGB16 color = interpolate_palette_hue(palette_position);
...
// Main color selection function that respects color mode
CRGB16 get_mode_color(SQ15x16 hue, SQ15x16 saturation, SQ15x16 value) {
    // DEBUG: Print mode being used occasionally
    static uint32_t last_debug = 0;
    if (millis() - last_debug > 2000) {
        USBSerial.print("COLOR MODE DEBUG: mode=");
        USBSerial.print(CONFIG.COLOR_MODE);
        USBSerial.print(" (HSV=0,PAL=1,HYB=2) hue=");
        USBSerial.print(float(hue));
        USBSerial.print(" value=");
        USBSerial.println(float(value));
        last_debug = millis();
    }
    ...
    switch (CONFIG.COLOR_MODE) {
        ...
        case COLOR_MODE_PALETTE: {
            // Use hue directly as palette position
            CRGB16 pal_color = interpolate_palette_hue(hue);
            pal_color.r *= value;
            pal_color.g *= value;
            pal_color.b *= value;
            return pal_color;
        ...
        case COLOR_MODE_HYBRID: {
            CRGB16 hsv_color = hsv(hue, saturation, value);
            CRGB16 pal_color = interpolate_palette_hue(hue);
            pal_color.r *= value;
            pal_color.g *= value;
            pal_color.b *= value;
            return blend_colors(hsv_color, pal_color, CONFIG.PALETTE_BLEND);
        }
