...
void light_mode_gdft() {
  ...
  for (uint16_t i = 0; i < (NATIVE_RESOLUTION / 2); i++) {
    ...
    leds_16[i + (NATIVE_RESOLUTION / 2)] = get_mode_color(led_hue + bin * SQ15x16(0.050), frame_config.SATURATION, bin);
  }
...
void light_mode_quantum_collapse() {
  ...
  for (uint16_t i = 0; i < NATIVE_RESOLUTION; i++) {
    ...
    leds_16[i] = hsv(field_hue, saturation, brightness);
  }
  ...
  for (uint8_t i = 0; i < 12; i++) {
    ...
    if (pos < NATIVE_RESOLUTION) {
      ...
      CRGB16 particle_color = hsv(particle_hue, 
