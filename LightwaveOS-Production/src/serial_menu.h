...
void parse_command(char* command_buf) {
  ...
  // Print help ---------------------------------------------
    ...
    USBSerial.println("                        set_palette/PAL=[int] | Set the palette index (0-32)");
    USBSerial.println("               color_mode=[hsv/palette/hybrid] | Set color mode");
    USBSerial.println("                       palette_blend=[0-255] | Set blend amount for hybrid mode");
    USBSerial.println("                          get_palette_count | Get number of available palettes");
    USBSerial.println("                      get_palette_name=[int] | Get palette name by index");
  ...
  // Cycle to next palette ----------------------------------
  else if (strcmp(command_buf, "P") == 0 || strcmp(command_buf, "p") == 0) {
    extern const uint8_t gGradientPaletteCount;
    CONFIG.PALETTE_INDEX = (CONFIG.PALETTE_INDEX + 1) % gGradientPaletteCount;
    gCurrentPaletteIndex = CONFIG.PALETTE_INDEX;
    load_palette_safe(gCurrentPaletteIndex);
    ...
    USBSerial.print(" - ");
    USBSerial.println(paletteNames[CONFIG.PALETTE_INDEX]);
    tx_end();
  }
  
  // Toggle color mode (Shift+P) ----------------------------
  else if (strcmp(command_buf, "P!") == 0) {
    CONFIG.COLOR_MODE = (CONFIG.COLOR_MODE + 1) % 3;
    
    save_config_delayed();
    tx_begin();
    USBSerial.print("COLOR_MODE: ");
    const char* mode_names[] = {"HSV", "PALETTE", "HYBRID"};
    USBSerial.println(mode_names[CONFIG.COLOR_MODE]);
    tx_end();
  }
  ...
  // COMMANDS WITH METADATA ##################################
    ...
    // Set Palette Index --------------------------------------
    else if (strcmp(command_type, "set_palette") == 0 || strcmp(command_type, "PAL") == 0) {
      extern const uint8_t gGradientPaletteCount;
      uint8_t new_index = constrain(atol(command_data), 0, gGradientPaletteCount - 1);
      CONFIG.PALETTE_INDEX = new_index;
      gCurrentPaletteIndex = new_index;
      load_palette_safe(gCurrentPaletteIndex);
      ...
      extern const char* const paletteNames[];
      USBSerial.print(" (");
      USBSerial.print(paletteNames[CONFIG.PALETTE_INDEX]);
      USBSerial.println(")");
      tx_end();
    }
    
    // Set Color Mode (HSV/Palette/Hybrid) --------------------
    else if (strcmp(command_type, "color_mode") == 0) {
      bool good = false;
      if (strcmp(command_data, "hsv") == 0 || strcmp(command_data, "0") == 0) {
        CONFIG.COLOR_MODE = COLOR_MODE_HSV;
        good = true;
      } else if (strcmp(command_data, "palette") == 0 || strcmp(command_data, "1") == 0) {
        CONFIG.COLOR_MODE = COLOR_MODE_PALETTE;
        good = true;
      } else if (strcmp(command_data, "hybrid") == 0 || strcmp(command_data, "2") == 0) {
        CONFIG.COLOR_MODE = COLOR_MODE_HYBRID;
        good = true;
      } else {
        bad_command(command_type, command_data);
      }
    ...
    // Set Palette Blend Amount -------------------------------
    else if (strcmp(command_type, "palette_blend") == 0) {
      CONFIG.PALETTE_BLEND = constrain(atol(command_data), 0, 255);
      
      save_config_delayed();
      tx_begin();
      USBSerial.print("CONFIG.PALETTE_BLEND: ");
      USBSerial.print(CONFIG.PALETTE_BLEND);
      USBSerial.print(" (");
      USBSerial.print((CONFIG.PALETTE_BLEND * 100) / 255);
      USBSerial.println("%)");
      tx_end();
    }
    
    // Get Palette Count --------------------------------------
    else if (strcmp(command_type, "get_palette_count") == 0) {
      extern const uint8_t gGradientPaletteCount;
      tx_begin();
      USBSerial.print("PALETTE_COUNT: ");
      USBSerial.println(gGradientPaletteCount);
      tx_end();
    }
    
    // Get Palette Name ---------------------------------------
    else if (strcmp(command_type, "get_palette_name") == 0) {
      extern const uint8_t gGradientPaletteCount;
      extern const char* const paletteNames[];
      uint8_t index = constrain(atol(command_data), 0, gGradientPaletteCount - 1);
      
      tx_begin();
      USBSerial.print("PALETTE_NAME: ");
      USBSerial.println(paletteNames[index]);
      tx_end();
    }
