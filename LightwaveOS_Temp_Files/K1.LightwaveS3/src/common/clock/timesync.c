/**
 * Time Synchronization Implementation (4-timestamp NTP-style)
 */
...
#define RTT_VARIANCE_THRESHOLD_US 20000  // 20ms to handle bimodal WiFi latency

// Maximum RTT considered valid (milliseconds)
#define MAX_VALID_RTT_MS 100  // 100ms to handle WiFi latency spikes
...
void lw_timesync_process_pong(
    ...
    printf("[TIMESYNC] ACCEPTED: delay=%lld us, offset=%lld us, samples=%u\n",
           (long long)delay_us, (long long)offset_est, (unsigned)(ts->good_samples + 1));
#endif
    ...
    // Removed debug spam for low-latency WS callbacks
