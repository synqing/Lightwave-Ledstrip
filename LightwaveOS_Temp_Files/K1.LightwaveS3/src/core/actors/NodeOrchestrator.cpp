...
bool NodeOrchestrator::init()
{
    ...
    try {
        ...
        // Create ShowNode (standalone mode)
...
#else
        // Node runtime: the Hub owns show playback.
        m_showDirector.reset();
#ifndef NATIVE_BUILD
        ESP_LOGI(TAG, "Node runtime mode: skipping ShowNode allocation");
#endif
#endif

#if FEATURE_AUDIO_SYNC && !FEATURE_NODE_RUNTIME
...
bool NodeOrchestrator::start()
{
    ...
    // Start ShowNode (standalone mode only).
    // In node-runtime mode the Hub is the authoritative show runner; running the
    // local show director wastes RAM and can prevent the node from booting.
#if !FEATURE_NODE_RUNTIME
...
#else
#ifndef NATIVE_BUILD
    ESP_LOGI(TAG, "Node runtime mode: skipping ShowNode");
#endif
#endif
