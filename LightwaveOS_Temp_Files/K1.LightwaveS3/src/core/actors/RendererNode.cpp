...
#include "../../node_integration.h"  // For nodeApplyScheduledCommands()
...
{
    // ==== CRITICAL: Apply scheduled commands from Node coordinator BEFORE rendering ====
    // This must be called at frame boundary to apply Hub-synchronized commands
    // (e.g. scene changes, parameter updates) without blocking
    #ifdef FEATURE_NODE_RUNTIME
    nodeApplyScheduledCommands();  // From node_integration.h (included above)
    #endif
