/**
 * @file LGPChromaticLensEffect.cpp
 * @brief LGP Chromatic Lens effect implementation
 */

#include "LGPChromaticLensEffect.h"
...
CRGB chromaticDispersion(float position, float aberration, float phase, float intensity) {
    // Normalise position from centre (0.0 = centre, 1.0 = edge)
...
void LGPChromaticLensEffect::render(plugins::EffectContext& ctx) {
    ...
    for (int i = 0; i < STRIP_LENGTH && i < (int)ctx.ledCount; i++) {
        CRGB color = chromaticDispersionPalette((float)i, aberration, m_phase, intensity, ctx.palette, ctx.gHue);
        ctx.leds[i] = color;
    }
...
void LGPChromaticLensEffect::cleanup() {
    // No resources to free
}

const plugins::EffectMetadata& LGPChromaticLensEffect::getMetadata() const {
    static plugins::EffectMetadata meta{
        "LGP Chromatic Lens",
        "Simulated lens dispersion",
