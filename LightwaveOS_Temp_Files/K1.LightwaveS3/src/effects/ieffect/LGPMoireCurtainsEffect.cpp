...
void LGPMoireCurtainsEffect::render(plugins::EffectContext& ctx) {
    ...
    const float baseFreq = 6.0f + complexityNorm * 8.0f;
    const float delta = 0.1f + variationNorm * 0.4f;

    const float leftFreq = baseFreq + delta / 2.0f;
    const float rightFreq = baseFreq - delta / 2.0f;

    m_phase = (uint16_t)(m_phase + ctx.speed);
    uint8_t hueOffset = (uint8_t)(variationNorm * 72.0f);
    ...
    for (uint16_t i = 0; i < STRIP_LENGTH; i++) {
        ...
        ctx.leds[i] = ctx.palette.getColor(
            (uint8_t)(ctx.gHue + hueOffset + (uint8_t)(distFromCenter / 2.0f)),
            leftBright);
        ...
        if (i + STRIP_LENGTH < ctx.ledCount) {
            ...
            ctx.leds[i + STRIP_LENGTH] = ctx.palette.getColor(
                (uint8_t)(ctx.gHue + hueOffset + (uint8_t)(distFromCenter / 2.0f) + 128),
                rightBright);
        }
    }
}
