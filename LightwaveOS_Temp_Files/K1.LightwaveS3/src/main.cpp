...
#include "node_integration.h"  // LightwaveOS Node coordinator

// Define Node integration globals (declared extern in node_integration.h)
NodeMain g_nodeMain;
uint16_t g_currentEffectId = 0;
uint16_t g_currentPaletteId = 0;
...
#if FEATURE_NODE_RUNTIME
static void printNodeRuntimeSerialHelp() {
    Serial.println("\nNode runtime serial controls:");
    Serial.println("  ? / help           - This help");
    Serial.println("  s / status         - Print status now");
    Serial.println("  l                  - List effects");
    Serial.println("  P                  - List palettes");
    Serial.println("  zones status       - ZoneComposer status");
    Serial.println("  zones off          - Disable ZoneComposer (debug)");
    Serial.println("  local status       - Show local-control mode");
    Serial.println("  local on/off       - Enable/disable local control override");
    Serial.println();
    Serial.println("Local override hotkeys (only when local is ON):");
    Serial.println("  SPACE / n / N      - Next / next / prev effect");
    Serial.println("  , / .              - Prev / next palette");
    Serial.println("  + / -              - Brightness up/down");
    Serial.println("  [ / ]              - Speed down/up");
    Serial.println("  z                  - Toggle ZoneComposer enable");
    Serial.println();
    Serial.println("Local override set commands (only when local is ON):");
    Serial.println("  e <id>             - Set effect ID");
    Serial.println("  p <id>             - Set palette ID");
    Serial.println("  b <0-255>          - Set brightness");
    Serial.println("  sp <0-255>         - Set speed");
    Serial.println();
    Serial.println("Note: In node runtime, the hub remains authoritative and may overwrite local changes.");
    Serial.println();
}

static uint8_t findNextActiveEffect(RendererNode* rendererNode, uint8_t start, int dir) {
    if (!rendererNode) return start;
    const uint8_t count = rendererNode->getEffectCount();
    if (count == 0) return start;

    for (uint8_t i = 0; i < count; ++i) {
        const uint8_t idx = (uint8_t)((start + (uint8_t)(dir > 0 ? i + 1 : count - i - 1)) % count);
        if (rendererNode->getEffectInstance(idx) != nullptr) {
            return idx;
        }
    }
    return start;
...
static void printNodeRuntimeStatus(RendererNode* rendererNode) {
    if (!rendererNode) {
        Serial.println("[Status] Renderer not ready");
        return;
    }

    const RenderStats& stats = rendererNode->getStats();
    if (zoneComposer.isEnabled()) {
        ...
        const uint8_t effectId = rendererNode->getCurrentEffect();
        Serial.printf("[Status] FPS: %d, CPU: %d%%, Effect: %s (ID %u)\n",
                      stats.currentFPS,
                      stats.cpuPercent,
                      rendererNode->getEffectName(effectId),
                      (unsigned)effectId);
    }
}

static void nodeRuntimeApplyImmediate(
    char c,
    bool localControlEnabled,
    NodeOrchestrator& orch,
    ...
    if (!localControlEnabled) {
        if (c == '?' || c == 's' || c == 'l' || c == 'P') {
            // Allowed even when local override is off.
        } else {
            Serial.println("[LOCAL] Disabled. Type: local on");
            return;
        }
    }

    const uint8_t effectCount = rendererNode->getEffectCount();
    const uint8_t paletteCount = rendererNode->getPaletteCount();

    switch (c) {
        case ' ':  // next effect
        case 'n': {
            if (effectCount == 0) return;
            const uint8_t current = rendererNode->getCurrentEffect();
            const uint8_t next = findNextActiveEffect(rendererNode, current, +1);
            orch.setEffect(next);
            Serial.printf("[LOCAL] effect=%u\n", (unsigned)next);
            break;
        }
        case 'N': { // prev effect
            if (effectCount == 0) return;
            const uint8_t cur = rendererNode->getCurrentEffect();
            const uint8_t prev = findNextActiveEffect(rendererNode, cur, -1);
            orch.setEffect(prev);
            Serial.printf("[LOCAL] effect=%u\n", (unsigned)prev);
            break;
        }
        case '+':
        case '=': {
            const uint8_t b = rendererNode->getBrightness();
            const uint8_t nb = (b == 255) ? 255 : (uint8_t)(b + 1);
            orch.setBrightness(nb);
            Serial.printf("[LOCAL] brightness=%u\n", (unsigned)nb);
            break;
        }
        case '-':
        case '_': {
            const uint8_t b = rendererNode->getBrightness();
            const uint8_t nb = (b == 0) ? 0 : (uint8_t)(b - 1);
            orch.setBrightness(nb);
            Serial.printf("[LOCAL] brightness=%u\n", (unsigned)nb);
            break;
        }
        case ']': {
            const uint8_t s = rendererNode->getSpeed();
            const uint8_t ns = (s == 255) ? 255 : (uint8_t)(s + 1);
            orch.setSpeed(ns);
            Serial.printf("[LOCAL] speed=%u\n", (unsigned)ns);
            break;
        }
        case '[': {
            const uint8_t s = rendererNode->getSpeed();
            const uint8_t ns = (s == 0) ? 0 : (uint8_t)(s - 1);
            orch.setSpeed(ns);
            Serial.printf("[LOCAL] speed=%u\n", (unsigned)ns);
            break;
        }
        case '.': {
            ...
            const uint8_t next = (rendererNode->getPaletteIndex() + 1) % paletteCount;
            orch.setPalette(next);
            Serial.printf("[LOCAL] palette=%u\n", (unsigned)next);
            break;
        }
        case ',': {
            if (paletteCount == 0) return;
            const uint8_t cur = rendererNode->getPaletteIndex();
            const uint8_t prev = (cur + paletteCount - 1) % paletteCount;
            orch.setPalette(prev);
            Serial.printf("[LOCAL] palette=%u\n", (unsigned)prev);
            break;
        }
        case 'z': {
            zoneComposer.setEnabled(!zoneComposer.isEnabled());
            Serial.printf("[LOCAL] zones=%s\n", zoneComposer.isEnabled() ? "ON" : "OFF");
            break;
        }
        default:
            break;
...
static void processNodeRuntimeSerial(
    uint32_t nowMs,
    bool& localControlEnabled,
    NodeOrchestrator& orch,
    RendererNode* rendererNode
    ...
    while (Serial.available()) {
        ...
        if (len == 0) {
            switch (c) {
                ...
                case ',': case '.':
                case 'z':
                    nodeRuntimeApplyImmediate(c, localControlEnabled, orch, rendererNode);
                    continue;
                default:
                    break;
            }
    ...
    // Lowercase copy in place for command matching (keeps numbers intact).
    for (char* p = s; *p; ++p) {
        *p = (char)tolower((unsigned char)*p);
    }

    if (strcmp(s, "?") == 0 || strcmp(s, "help") == 0) {
        printNodeRuntimeSerialHelp();
        return;
    }
    if (strcmp(s, "s") == 0 || strcmp(s, "status") == 0) {
        printNodeRuntimeStatus(rendererNode);
        return;
    }
    if (strcmp(s, "l") == 0) {
        if (!rendererNode) { Serial.println("Renderer not ready"); return; }
        const uint8_t count = rendererNode->getEffectCount();
        Serial.printf("Effects (%u):\n", (unsigned)count);
        for (uint8_t i = 0; i < count; ++i) {
            Serial.printf("  %3u: %s\n", (unsigned)i, rendererNode->getEffectName(i));
        }
        return;
    }
    if (strcmp(s, "p") == 0) {
        // Keep legacy 'P' list palettes behaviour: accept lowercase after normalisation.
        if (!rendererNode) { Serial.println("Renderer not ready"); return; }
        const uint8_t count = rendererNode->getPaletteCount();
        Serial.printf("Palettes (%u):\n", (unsigned)count);
        for (uint8_t i = 0; i < count; ++i) {
            Serial.printf("  %3u: %s\n", (unsigned)i, rendererNode->getPaletteName(i));
        }
        return;
    }
    ...
    if (strcmp(cmd, "local") == 0) {
        ...
        Serial.println("Usage: local on|off|status");
        return;
    }

    if (strcmp(cmd, "zones") == 0) {
        if (!arg1 || strcmp(arg1, "status") == 0) {
            Serial.printf("[ZONES] %s (%d zones)\n",
                          zoneComposer.isEnabled() ? "ON" : "OFF",
                          (int)zoneComposer.getZoneCount());
            return;
        }
        if (strcmp(arg1, "off") == 0) {
            zoneComposer.setEnabled(false);
            Serial.println("[ZONES] OFF");
            return;
        }
        if (strcmp(arg1, "on") == 0) {
            zoneComposer.setEnabled(true);
            Serial.println("[ZONES] ON");
            return;
        }
        Serial.println("Usage: zones on|off|status");
        return;
    }
    ...
    // Mutating commands (local override required).
    if ((strcmp(cmd, "e") == 0 || strcmp(cmd, "effect") == 0) && arg1) {
        const uint32_t id = (uint32_t)strtoul(arg1, nullptr, 10);
        if (!rendererNode || rendererNode->getEffectInstance((uint8_t)id) == nullptr) {
            Serial.printf("ERROR: Invalid effect ID: %lu\n", (unsigned long)id);
            return;
        }
        orch.setEffect((uint8_t)id);
        Serial.printf("[LOCAL] effect=%lu\n", (unsigned long)id);
        return;
    }
    if ((strcmp(cmd, "p") == 0 || strcmp(cmd, "palette") == 0) && arg1) {
        const uint32_t id = (uint32_t)strtoul(arg1, nullptr, 10);
        orch.setPalette((uint8_t)id);
        Serial.printf("[LOCAL] palette=%lu\n", (unsigned long)id);
        return;
    }
    if ((strcmp(cmd, "b") == 0 || strcmp(cmd, "brightness") == 0) && arg1) {
        const uint32_t v = (uint32_t)strtoul(arg1, nullptr, 10);
        orch.setBrightness((uint8_t)v);
        Serial.printf("[LOCAL] brightness=%lu\n", (unsigned long)v);
        return;
    }
    if ((strcmp(cmd, "sp") == 0 || strcmp(cmd, "speed") == 0) && arg1) {
        const uint32_t v = (uint32_t)strtoul(arg1, nullptr, 10);
        orch.setSpeed((uint8_t)v);
        Serial.printf("[LOCAL] speed=%lu\n", (unsigned long)v);
        return;
    }
...
#else
    LW_LOGI("Node runtime: skipping NVS/Narrative init (Hub-owned state)");

    // ZoneComposer is still required in node-runtime so the hub can drive per-zone
    // rendering via `zones.*` commands. We initialise it without persistence.
    LW_LOGI("Initialising Zone Composer (node runtime)...");
    if (!zoneComposer.init(renderer)) {
        LW_LOGE("Zone Composer init failed (node runtime)");
    } else {
        renderer->setZoneComposer(&zoneComposer);
        // Keep disabled until the hub sends an explicit zone update/snapshot.
        zoneComposer.setEnabled(false);
        LW_LOGI("Zone Composer: INITIALISED (node runtime)");
    }
#endif

#if FEATURE_NODE_RUNTIME
    // =========================================================================
    // Initialise LightwaveOS Node Coordinator (Hub-controlled runtime)
    // =========================================================================
    // Must run before the actor system starts spawning additional tasks, so WiFi
    // and WS can initialise with a healthy heap.
    LW_LOGI("==========================================");
    LW_LOGI("Initialising LightwaveOS Node coordinator...");
    LW_LOGI("==========================================");

    // Wire Node to v2 Actor system
    if (!initNodeCoordinator(&orchestrator)) {
        LW_LOGE("Node coordinator init failed!");
        LW_LOGE("Hub control plane will be unavailable");
    } else {
        LW_LOGI("Node coordinator initialised successfully");
        LW_LOGI("  Network Control Adapter: ACTIVE");
        LW_LOGI("  Hub commands will control v2 renderer via Actor messages");
    }
#endif
...
#else
    // Node runtime: Hub sends authoritative state after connect.
    orchestrator.setEffect(0);       // Fire
    orchestrator.setBrightness(128); // 50% brightness
    orchestrator.setSpeed(15);       // Medium speed
    orchestrator.setPalette(0);      // Party colours
    LW_LOGI("Node runtime: using defaults, awaiting Hub state");
#endif

    // Initialize Network (if enabled)
#if FEATURE_WEB_SERVER
    // In Node runtime mode, Tab5 is the Hub (SoftAP host) and this device must join as STA.
    // Starting WiFiManager in AP mode here would override NodeMain's WiFi STA and prevent
    // HELLO/WELCOME, which blocks the entire Hub→Node control plane.
#if !FEATURE_NODE_RUNTIME
...
#else
    LW_LOGI("WiFiManager: DISABLED (Node runtime mode - WiFi STA managed by Node coordinator)");
#endif

    // Start WebServer (only in standalone mode, not Node runtime)
#if !FEATURE_NODE_RUNTIME
    LW_LOGI("Starting Web Server (standalone mode)...");
...
#else
    LW_LOGI("Web Server: DISABLED (Node runtime mode - controlled by Hub)");
#endif
...
void loop() {
...
#if FEATURE_NODE_RUNTIME
    // Node runtime: keep serial access for diagnostics, and optionally allow local overrides.
    // Local overrides are OFF by default to avoid fighting the Hub control plane.
    static bool s_localControlEnabled = false;
    processNodeRuntimeSerial(now, s_localControlEnabled, orchestrator, renderer);
#endif

#if !FEATURE_NODE_RUNTIME
    // ========================================================================
    // LEGACY LOCAL CONTROLS (disabled when running as Node runtime)
    // ========================================================================
    // When FEATURE_NODE_RUNTIME is enabled, Hub/Node wireless architecture
    // is the primary control plane. Local controls (encoder/serial) are
    // disabled to prevent control-plane conflicts.
    // ========================================================================
...
#endif  // !FEATURE_NODE_RUNTIME (end of legacy local controls gate)
    ...
    if (now - lastStatus > 10000) {
        ...
        if (zoneComposer.isEnabled()) {
            ...
            const uint8_t liveEffect = renderer ? renderer->getCurrentEffect() : currentEffect;
            Serial.printf("[Status] FPS: %d, CPU: %d%%, Effect: %s\n",
                          stats.currentFPS,
                          stats.cpuPercent,
                          renderer ? renderer->getEffectName(liveEffect) : "Unknown");
        }
    ...
    // Update WebServer (if enabled and not Node runtime)
    // Note: WiFiManager runs on its own FreeRTOS task
#if FEATURE_WEB_SERVER && !FEATURE_NODE_RUNTIME
    if (webServerInstance) {
