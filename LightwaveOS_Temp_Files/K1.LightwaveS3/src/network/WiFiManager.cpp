...
bool WiFiManager::begin() {
    ...
    // STA+AP architecture: Run both Station and Access Point simultaneously
    // This allows Tab5.encoder to connect via home WiFi OR directly via AP
    bool hasCredentials = !m_ssid.isEmpty() && m_ssid != "CONFIGURE_ME";

    if (hasCredentials) {
        // Enable STA+AP mode - connect to WiFi AND run AP for fallback
        WiFi.mode(WIFI_MODE_APSTA);
        m_apEnabled = true;
        m_forceApModeRuntime = false;
        LW_LOGI("STA+AP mode enabled (credentials available)");
    } else {
        // No credentials - AP-only mode
        WiFi.mode(WIFI_MODE_AP);
        m_apEnabled = true;
        m_forceApModeRuntime = true;
        LW_LOGI("AP-only mode (no credentials configured)");
    }

    LW_LOGI("  AP SSID: '%s'", m_apSSID.c_str());

    // Start AP immediately (always available as fallback)
    if (!m_apSSID.isEmpty()) {
        LW_LOGI("Starting Soft-AP in %s mode: '%s'",
                hasCredentials ? "STA+AP" : "AP-only",
                m_apSSID.c_str());
...
void WiFiManager::handleStateInit() {
    ...
    // STA+AP architecture: Auto-connect to WiFi when credentials are available
    // forceApModeRuntime is false when credentials exist (set in begin())
    if (m_forceApModeRuntime || m_ssid.isEmpty() || m_ssid == "CONFIGURE_ME") {
        // No credentials or AP-only mode forced - stay in AP mode
        LW_LOGI("No credentials or AP-only mode - staying in AP mode");
        setState(STATE_WIFI_AP_MODE);
        return;
    }

    // Credentials available - proceed with STA connection
...
void WiFiManager::handleStateAPMode() {
    ...
    if (millis() - lastStatusPrint > 30000) {
        lastStatusPrint = millis();
        LW_LOGI("AP Mode - SSID: '%s', IP: %s, Channel: %d, Clients: %d",
                m_apSSID.c_str(),
                WiFi.softAPIP().toString().c_str(),
                m_apChannel,
