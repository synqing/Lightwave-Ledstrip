...
if not toolchain_dir:
    ...
    # Prefer the newest available toolchain (esp-14.x) over legacy esp-8.x.
    def _read_pkg_version(dir_path: str) -> str:
        try:
    ...
    def _ver_key(v: str):
        # "14.2.0+20241119" -> (14, 2, 0, 20241119)
        ...
        while len(parts) < 3:
            parts.append(0)
        try:
            parts.append(int(build) if build else 0)
        except Exception:
            parts.append(0)
        return tuple(parts)
    ...
    if os.path.isdir(pio_packages_dir):
        ...
        for item in os.listdir(pio_packages_dir):
            ...
            if _has_compiler(candidate):
                candidates.append((candidate, _read_pkg_version(candidate)))
        if candidates:
            candidates.sort(key=lambda t: _ver_key(t[1]), reverse=True)
            toolchain_dir = candidates[0][0]
