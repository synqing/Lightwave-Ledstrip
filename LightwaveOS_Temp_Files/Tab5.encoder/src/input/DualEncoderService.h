...
public:
    ...
    /**
     * Return the last sampled raw button state for an encoder (updated in update()).
     * This is intended for higher-level input handling (e.g., presets) without
     * performing additional I2C reads outside the encoder service.
     */
    bool isButtonPressed(uint8_t globalIdx) const {
        return (globalIdx < TOTAL_ENCODERS) ? _rawPressed[globalIdx] : false;
    }
...
private:
    ...
    bool _rawPressed[TOTAL_ENCODERS] = {false};
    ...
    , _transportB(wire, addressB)  // Same bus, address 0x41 (factory)
{
    ...
    _values[5] = ParamDefault::COMPLEXITY;
    _values[6] = ParamDefault::VARIATION;
    _values[7] = ParamDefault::BRIGHTNESS;
...
inline void DualEncoderService::update() {
    ...
    if (_transportA.isAvailable()) {
        for (uint8_t localIdx = 0; localIdx < ENCODERS_PER_UNIT; localIdx++) {
            ...
            _rawPressed[globalIdx] = isPressed;
...
inline void DualEncoderService::resetToDefaults(bool triggerCallbacks) {
    ...
    // CRITICAL: Order must match Parameter enum and ParameterMap!
    _values[0] = ParamDefault::EFFECT;
    _values[1] = ParamDefault::PALETTE;      // Index 1 = Palette (0)
    ...
    if (_detentDebounce[globalIdx].processRawDelta(rawDelta, now)) {
        ...
        if (normalizedDelta != 0) {
            ...
            Serial.printf("[DEBUG] {\"sessionId\":\"debug-session\",\"runId\":\"run1\",\"hypothesisId\":\"WRAP2\",\"location\":\"DualEncoderService.h:503\",\"message\":\"processEncoderDelta\",\"data\":{\"globalIdx\":%d,\"oldValue\":%d,\"normalizedDelta\":%ld,\"newValueBeforeConstraint\":%ld,\"newValueAfterConstraint\":%d,\"shouldWrap\":%d},\"timestamp\":%lu}\n",
                          globalIdx, oldValue, (long)normalizedDelta, (long)newValue, _values[globalIdx],
                          shouldWrapGlobal(globalIdx) ? 1 : 0, (unsigned long)now);
            #endif
...
inline uint16_t DualEncoderService::getDefaultValue(uint8_t globalIdx) const {
    const ParameterDef* param = getParameterByIndex(globalIdx);
    if (!param) {
        return 0;
    }
...
inline uint16_t DualEncoderService::applyRangeConstraint(uint8_t globalIdx, int32_t value) const {
    ...
    int32_t minVal = (int32_t)getParameterMin(globalIdx);
    int32_t maxVal = (int32_t)getParameterMax(globalIdx);

    // Zone speed/palette encoders (9/11/13/15) use a runtime range based on toggle mode.
    if (ZoneParam::isZoneParameter(globalIdx) && ZoneParam::isZoneBrightness(globalIdx)) {
        const uint8_t zoneId = ZoneParam::getZoneId(globalIdx);
        const bool paletteMode = (_buttonHandler && _buttonHandler->getZoneEncoderMode(zoneId) == SpeedPaletteMode::PALETTE);
        if (paletteMode) {
            minVal = (int32_t)ParamRange::ZONE_PALETTE_MIN;
            maxVal = (int32_t)ParamRange::ZONE_PALETTE_MAX;
        } else {
            minVal = (int32_t)ParamRange::ZONE_SPEED_MIN;
            maxVal = (int32_t)ParamRange::ZONE_SPEED_MAX;
        }
    }
    ...
    if (shouldWrapGlobal(globalIdx)) {
        ...
        if (range <= 0) {
            return (uint16_t)minVal;
        }
        int32_t wrapped = value - minVal;
        wrapped %= range;
        if (wrapped < 0) wrapped += range;
        return (uint16_t)(minVal + wrapped);
    }

    if (value < minVal) return (uint16_t)minVal;
    if (value > maxVal) return (uint16_t)maxVal;
...
inline bool DualEncoderService::shouldWrapGlobal(uint8_t globalIdx) const {
    ...
    // Unit B: Zone Effect selectors always wrap; speed encoders wrap only when toggled to Palette mode.
    if (!ZoneParam::isZoneParameter(globalIdx)) {
        return false;
    }
    if (ZoneParam::isZoneEffect(globalIdx)) {
        return true;
    }
    if (ZoneParam::isZoneBrightness(globalIdx)) {
        const uint8_t zoneId = ZoneParam::getZoneId(globalIdx);
        return (_buttonHandler && _buttonHandler->getZoneEncoderMode(zoneId) == SpeedPaletteMode::PALETTE);
    }
    return false;
}
