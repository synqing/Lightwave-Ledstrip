...
inline uint16_t wrapValue(uint8_t param, int32_t value) {
    ...
    if (originalValue != value) {
        Serial.printf(
            "[DEBUG] {\"sessionId\":\"debug-session\",\"runId\":\"run1\",\"hypothesisId\":\"WRAP1\",\"location\":\"EncoderProcessing.h:wrapValue\",\"message\":\"wrapped\",\"data\":{\"param\":%d,\"originalValue\":%ld,\"wrappedValue\":%ld,\"minVal\":%d,\"maxVal\":%d,\"range\":%ld},\"timestamp\":%lu}\n",
...
inline bool shouldWrap(uint8_t param) {
    ...
    // Effect (0) and Palette (1) wrap; everything else clamps
