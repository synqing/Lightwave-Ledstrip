...
void I2CRecovery::init(TwoWire* wire, int sda, int scl, uint32_t freq) {
    ...
    Serial.printf(COLOR_I2C_RECOVERY "[I2C_RECOVERY]" ANSI_RESET " Initialized: SDA=%d SCL=%d freq=%luHz\n",
...
void I2CRecovery::recordError() {
    ...
    if (s_errorCount >= ERROR_THRESHOLD - 1) {
        Serial.printf(COLOR_I2C_RECOVERY "[I2C_RECOVERY]" ANSI_RESET " Error count: %d/%d\n",
...
bool I2CRecovery::detectBusHang() {
    ...
    // Read line level without changing pin mux/function.
    // Using pinMode() here can deconfigure the I2C peripheral on Arduino-ESP32,
    // leading to "bus is not initialized" errors.
    bool sdaLow = (gpio_get_level(static_cast<gpio_num_t>(s_sda)) == 0);

    // If SDA is stuck low, the bus is hung
    if (sdaLow) {
        Serial.println(COLOR_I2C_RECOVERY "[I2C_RECOVERY]" ANSI_RESET " Bus hang detected: SDA stuck LOW");
    }
...
bool I2CRecovery::isBusHealthy() {
    ...
    // Read line levels without changing pin mux/function.
    bool sdaHigh = (gpio_get_level(static_cast<gpio_num_t>(s_sda)) != 0);
    bool sclHigh = (gpio_get_level(static_cast<gpio_num_t>(s_scl)) != 0);
...
void I2CRecovery::forceRecovery() {
    ...
    Serial.println(COLOR_I2C_RECOVERY "[I2C_RECOVERY]" ANSI_RESET " Starting recovery sequence...");
    ...
    switch (s_stage) {
        ...
        case RecoveryStage::CheckSDA:
            ...
            if (digitalRead(s_sda) == LOW) {
                Serial.println(COLOR_I2C_RECOVERY "[I2C_RECOVERY]" ANSI_RESET " SDA stuck low - toggling SCL");
                s_sclToggleCount = 0;
                advanceTo(RecoveryStage::SclToggle, STEP_DELAY_SHORT_MS);
            } else {
                // SDA is already high, just send STOP and reinit
                Serial.println(COLOR_I2C_RECOVERY "[I2C_RECOVERY]" ANSI_RESET " SDA not stuck - sending STOP");
            ...
            // Do this in batches to stay non-blocking
            {
                ...
                for (uint8_t i = 0; i < 3 && s_sclToggleCount < SCL_TOGGLE_MAX; i++) {
                    ...
                    if (digitalRead(s_sda) == HIGH) {
                        Serial.printf(COLOR_I2C_RECOVERY "[I2C_RECOVERY]" ANSI_RESET " SDA released after %d SCL pulses\n",
                                      s_sclToggleCount);
                        advanceTo(RecoveryStage::StopCondition1, STEP_DELAY_SHORT_MS);
                        return;
                    }
                ...
                if (s_sclToggleCount >= SCL_TOGGLE_MAX) {
                    Serial.println(COLOR_I2C_RECOVERY "[I2C_RECOVERY]" ANSI_RESET " Max SCL toggles reached");
                    advanceTo(RecoveryStage::StopCondition1, STEP_DELAY_SHORT_MS);
                } else {
        ...
        case RecoveryStage::WaitAfterStop:
            ...
            if (s_softwareAttempts >= SOFTWARE_ATTEMPTS_BEFORE_HW) {
                Serial.println(COLOR_I2C_RECOVERY "[I2C_RECOVERY]" ANSI_RESET " Escalating to hardware peripheral reset");
        ...
        case RecoveryStage::HwPeriphReset:
            ...
            Serial.println(COLOR_I2C_RECOVERY "[I2C_RECOVERY]" ANSI_RESET " Executing i2c_ll_reset_register(0)");
        ...
        case RecoveryStage::HwWaitAfterReset:
            ...
            Serial.println(COLOR_I2C_RECOVERY "[I2C_RECOVERY]" ANSI_RESET " Hardware reset complete, reinitializing Wire");
            advanceTo(RecoveryStage::WireBegin, STEP_DELAY_SHORT_MS);
            break;
        ...
        case RecoveryStage::WireBegin:
            ...
            if (s_wire) {
                ...
                Serial.printf(COLOR_I2C_RECOVERY "[I2C_RECOVERY]" ANSI_RESET " Wire reinitialized at %luHz\n",
        ...
        case RecoveryStage::Verify:
            // Verify bus is healthy
            {
                ...
                if (healthy) {
                    Serial.println(COLOR_I2C_RECOVERY "[I2C_RECOVERY]" ANSI_RESET " Bus appears healthy");
                    completeRecovery(true);
                } else {
                    Serial.println(COLOR_I2C_RECOVERY "[I2C_RECOVERY]" ANSI_RESET " Bus still unhealthy after recovery");
...
void I2CRecovery::executeHardwareReset(int port) {
    ...
    Serial.printf(COLOR_I2C_RECOVERY "[I2C_RECOVERY]" ANSI_RESET " Hardware reset executed for I2C%d\n", port);
}
...
void I2CRecovery::completeRecovery(bool success) {
    ...
    if (success) {
        ...
        Serial.printf(COLOR_I2C_RECOVERY "[I2C_RECOVERY]" ANSI_RESET " Recovery SUCCESSFUL (%d/%d total)\n",
                      s_recoverySuccesses, s_recoveryAttempts);
    } else {
        s_softwareAttempts++;  // Increment for escalation on next attempt
        Serial.printf(COLOR_I2C_RECOVERY "[I2C_RECOVERY]" ANSI_RESET " Recovery FAILED (attempt %d, escalation level %d/%d)\n",
