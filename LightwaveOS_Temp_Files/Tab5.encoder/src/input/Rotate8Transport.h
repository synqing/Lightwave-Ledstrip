...
public:
    ...
    void setLED(uint8_t channel, uint8_t r, uint8_t g, uint8_t b) {
        ...
        if (!_available || channel > 8) {
            ...
            if (channel > 8) {
                Serial.printf(
                    "[I2C] Rotate8Transport setLED invalid channel=%u at %lu ms\n",
                    (unsigned)channel, (unsigned long)millis());
            }
            #endif
            return;
        }
    ...
    void setAllLEDs(uint8_t r, uint8_t g, uint8_t b) {
        if (!_available) return;
        #if ENABLE_ENCODER_DIAGNOSTICS
        Serial.printf("[I2C] Rotate8Transport allLEDs.set addr=0x%02X r=%u g=%u b=%u\n",
                      (unsigned)_address, (unsigned)r, (unsigned)g, (unsigned)b);
        #endif
    ...
    void allLEDsOff() {
        if (!_available) return;
        #if ENABLE_ENCODER_DIAGNOSTICS
        Serial.printf("[I2C] Rotate8Transport allLEDs.off addr=0x%02X\n", (unsigned)_address);
        #endif
        _encoder.allOff();
    }
