...
void TouchHandler::handleTouchStart(int16_t x, int16_t y) {
    ...
    if (now - m_lastEventTime < TouchConfig::DEBOUNCE_MS) {
        // #region agent log
        Serial.printf(COLOR_DEBUG "[DEBUG]" ANSI_RESET " {\"sessionId\":\"debug-session\",\"runId\":\"run1\",\"hypothesisId\":\"H5\",\"location\":\"TouchHandler.cpp:62\",\"message\":\"touch.start.debounced\",\"data\":{\"x\":%d,\"y\":%d,\"now\":%lu,\"lastEvent\":%lu,\"delta\":%lu},\"timestamp\":%lu}\n", x, y, (unsigned long)now, (unsigned long)m_lastEventTime, (unsigned long)(now - m_lastEventTime), (unsigned long)now);
        // #endregion
        return;
    }
    ...
    // #region agent log
    Serial.printf(COLOR_DEBUG "[DEBUG]" ANSI_RESET " {\"sessionId\":\"debug-session\",\"runId\":\"run1\",\"hypothesisId\":\"H3\",\"location\":\"TouchHandler.cpp:74\",\"message\":\"touch.start.zone\",\"data\":{\"x\":%d,\"y\":%d,\"zone\":%d,\"actionRowYStart\":%d,\"actionRowYEnd\":%d},\"timestamp\":%lu}\n", x, y, (int)zone, TouchConfig::ACTION_ROW_Y_START, TouchConfig::ACTION_ROW_Y_END, (unsigned long)now);
    // #endregion
    ...
    if (zone == TouchZone::PARAMETER_GRID) {
        ...
        if (m_touchedParam >= 0) {
            Serial.printf(COLOR_TOUCH "[TOUCH]" ANSI_RESET " Start on param %d at (%d,%d)\n",
                          m_touchedParam, x, y);
        }
    }
        ...
        // #region agent log
        Serial.printf(COLOR_DEBUG "[DEBUG]" ANSI_RESET " {\"sessionId\":\"debug-session\",\"runId\":\"run1\",\"hypothesisId\":\"H1,H2\",\"location\":\"TouchHandler.cpp:90\",\"message\":\"touch.start.actionRow\",\"data\":{\"x\":%d,\"y\":%d,\"touchedAction\":%d,\"buttonW\":%d,\"screenW\":%d,\"actionRowYStart\":%d,\"actionRowYEnd\":%d,\"calculatedButtonIdx\":%d},\"timestamp\":%lu}\n", x, y, m_touchedAction, TouchConfig::ACTION_BUTTON_W, TouchConfig::SCREEN_WIDTH, TouchConfig::ACTION_ROW_Y_START, TouchConfig::ACTION_ROW_Y_END, (x / TouchConfig::ACTION_BUTTON_W), (unsigned long)now);
        // #endregion
    }
    else if (zone == TouchZone::STATUS_BAR) {
        m_touchedParam = -1;
        Serial.printf(COLOR_TOUCH "[TOUCH]" ANSI_RESET " Start on status bar at (%d,%d)\n", x, y);
    }
    else {
        m_touchedParam = -1;
    }
...
void TouchHandler::handleTouchHold(int16_t x, int16_t y, uint32_t duration) {
    ...
    if (duration >= TouchConfig::LONG_PRESS_THRESHOLD_MS) {
        ...
        if (currentParam >= 0 && currentParam == m_touchedParam) {
            Serial.printf(COLOR_TOUCH "[TOUCH]" ANSI_RESET " Long press on param %d (duration %lu ms)\n",
...
void TouchHandler::handleTouchRelease(uint32_t duration) {
    ...
    // #region agent log
    Serial.printf(COLOR_DEBUG "[DEBUG]" ANSI_RESET " {\"sessionId\":\"debug-session\",\"runId\":\"run1\",\"hypothesisId\":\"H2\",\"location\":\"TouchHandler.cpp:134\",\"message\":\"touch.release.entry\",\"data\":{\"duration\":%lu,\"longPressTriggered\":%d,\"touchedParam\":%d,\"touchedAction\":%d,\"touchX\":%d,\"touchY\":%d},\"timestamp\":%lu}\n", (unsigned long)duration, m_longPressTriggered ? 1 : 0, m_touchedParam, m_touchedAction, m_touchX, m_touchY, (unsigned long)now);
    // #endregion
    ...
    if (m_longPressTriggered) {
        m_touchedParam = -1;
        // #region agent log
        Serial.printf(COLOR_DEBUG "[DEBUG]" ANSI_RESET " {\"sessionId\":\"debug-session\",\"runId\":\"run1\",\"hypothesisId\":\"H5\",\"location\":\"TouchHandler.cpp:140\",\"message\":\"touch.release.longPress\",\"data\":{\"duration\":%lu},\"timestamp\":%lu}\n", (unsigned long)duration, (unsigned long)now);
        // #endregion
        return;
    }
    ...
    if (duration <= TouchConfig::TAP_MAX_MS) {
        ...
        if (m_touchedParam >= 0) {
            Serial.printf(COLOR_TOUCH "[TOUCH]" ANSI_RESET " Tap on param %d (duration %lu ms)\n",
            ...
            // #region agent log
            Serial.printf(COLOR_DEBUG "[DEBUG]" ANSI_RESET " {\"sessionId\":\"debug-session\",\"runId\":\"run1\",\"hypothesisId\":\"H3\",\"location\":\"TouchHandler.cpp:158\",\"message\":\"touch.release.zoneCheck\",\"data\":{\"zone\":%d,\"touchX\":%d,\"touchY\":%d,\"actionRowYStart\":%d,\"actionRowYEnd\":%d},\"timestamp\":%lu}\n", (int)zone, m_touchX, m_touchY, TouchConfig::ACTION_ROW_Y_START, TouchConfig::ACTION_ROW_Y_END, (unsigned long)now);
            // #endregion
            if (zone == TouchZone::STATUS_BAR && m_statusBarCallback) {
                m_statusBarCallback(m_touchX, m_touchY);
            }
            // #region agent log
            Serial.printf(COLOR_DEBUG "[DEBUG]" ANSI_RESET " {\"sessionId\":\"debug-session\",\"runId\":\"run1\",\"hypothesisId\":\"H4\",\"location\":\"TouchHandler.cpp:181\",\"message\":\"touch.release.actionRowCheck\",\"data\":{\"zone\":%d,\"zoneIsActionRow\":%d,\"callbackSet\":%d,\"touchedAction\":%d,\"touchX\":%d,\"touchY\":%d,\"actionRowYStart\":%d,\"actionRowYEnd\":%d},\"timestamp\":%lu}\n", (int)zone, (zone == TouchZone::ACTION_ROW) ? 1 : 0, (m_actionButtonCallback != nullptr) ? 1 : 0, m_touchedAction, m_touchX, m_touchY, TouchConfig::ACTION_ROW_Y_START, TouchConfig::ACTION_ROW_Y_END, (unsigned long)now);
            // #endregion
            if (zone == TouchZone::ACTION_ROW && m_actionButtonCallback && m_touchedAction >= 0) {
                // #region agent log
                Serial.printf(COLOR_DEBUG "[DEBUG]" ANSI_RESET " {\"sessionId\":\"debug-session\",\"runId\":\"run1\",\"hypothesisId\":\"H4\",\"location\":\"TouchHandler.cpp:184\",\"message\":\"touch.release.callbackInvoke\",\"data\":{\"buttonIndex\":%d},\"timestamp\":%lu}\n", m_touchedAction, (unsigned long)now);
                // #endregion
                ...
                Serial.printf(COLOR_DEBUG "[DEBUG]" ANSI_RESET " {\"sessionId\":\"debug-session\",\"runId\":\"run1\",\"hypothesisId\":\"H1,H2,H3,H4\",\"location\":\"TouchHandler.cpp:189\",\"message\":\"touch.release.actionRowFailed\",\"data\":{\"zone\":%d,\"zoneIsActionRow\":%d,\"callbackSet\":%d,\"touchedAction\":%d,\"reason\":\"%s\",\"touchX\":%d,\"touchY\":%d},\"timestamp\":%lu}\n", (int)zone, (zone == TouchZone::ACTION_ROW) ? 1 : 0, (m_actionButtonCallback != nullptr) ? 1 : 0, m_touchedAction, reason, m_touchX, m_touchY, (unsigned long)now);
                // #endregion
            }
        }
    } else {
        // #region agent log
        Serial.printf(COLOR_DEBUG "[DEBUG]" ANSI_RESET " {\"sessionId\":\"debug-session\",\"runId\":\"run1\",\"hypothesisId\":\"H5\",\"location\":\"TouchHandler.cpp:144\",\"message\":\"touch.release.durationTooLong\",\"data\":{\"duration\":%lu,\"tapMax\":%lu},\"timestamp\":%lu}\n", (unsigned long)duration, (unsigned long)TouchConfig::TAP_MAX_MS, (unsigned long)now);
        // #endregion
    }
...
void TouchHandler::resetParameterToDefault(uint8_t paramIndex) {
    if (!m_encoderService) {
        Serial.printf(COLOR_TOUCH "[TOUCH]" ANSI_RESET " No encoder service - cannot reset param %d\n",
                      paramIndex);
        return;
    }
    ...
    Serial.printf(COLOR_TOUCH "[TOUCH]" ANSI_RESET " Reset param %d (%s) to default %d\n",
                  paramIndex,
