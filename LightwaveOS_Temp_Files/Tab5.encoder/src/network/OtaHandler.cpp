...
bool OtaHandler::checkOTAToken(AsyncWebServerRequest* request) {
    ...
    if (!request->hasHeader("X-OTA-Token")) {
        Serial.printf(COLOR_OTA "[OTA]" ANSI_RESET " Request missing X-OTA-Token header from %s\n",
    ...
    if (providedToken != expectedToken) {
        Serial.printf(COLOR_OTA "[OTA]" ANSI_RESET " Request with invalid token from %s\n",
                      request->client()->remoteIP().toString().c_str());
...
void OtaHandler::handleV1Update(AsyncWebServerRequest* request) {
    ...
    if (s_updateError) {
        Serial.printf(COLOR_OTA "[OTA]" ANSI_RESET " Update failed: %s\n", s_updateErrorMessage.c_str());
    ...
    Serial.println(COLOR_OTA "[OTA]" ANSI_RESET " Update successful, rebooting...");
...
void OtaHandler::handleUpload(AsyncWebServerRequest* request,
    ...
    if (index == 0) {
        Serial.printf(COLOR_OTA "[OTA]" ANSI_RESET " Upload starting: %s\n", filename.c_str());
        ...
        Serial.printf(COLOR_OTA "[OTA]" ANSI_RESET " Firmware size: %u bytes\n", s_updateTotal);
        ...
        if (s_updateTotal > freeSpace) {
            ...
            Serial.printf(COLOR_OTA "[OTA]" ANSI_RESET " ERROR: %s\n", s_updateErrorMessage.c_str());
            return;
        }
        ...
        Serial.printf(COLOR_OTA "[OTA]" ANSI_RESET " Update started, expecting %u bytes\n", s_updateTotal);
    }
    ...
    if (len > 0) {
        ...
        if (s_updateTotal > 0) {
            ...
            if (percent / 10 > lastPercent / 10) {
                Serial.printf(COLOR_OTA "[OTA]" ANSI_RESET " Progress: %u%%\n", percent);
                lastPercent = percent;
            }
        }
    }
    ...
    if (final) {
        Serial.println(COLOR_OTA "[OTA]" ANSI_RESET " Upload complete, finalizing...");
        ...
        Serial.println(COLOR_OTA "[OTA]" ANSI_RESET " Upload finalized successfully");
    }
}
