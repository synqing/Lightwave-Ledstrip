...
#include "../config/AnsiColors.h"  // ANSI color codes for debug output
    ...
    , _sendAttemptStartTime(0)
{
    ...
    if (_sendMutex == nullptr) {
        Serial.println(COLOR_WEBSOCKET "[WS]" ANSI_RESET " ERROR: Failed to create send mutex!");
    }
...
void WebSocketClient::begin(const char* host, uint16_t port, const char* path) {
    ...
    if (_status == WebSocketStatus::CONNECTING || _status == WebSocketStatus::CONNECTED) {
        Serial.println(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Already connected/connecting, ignoring begin()");
        return;
    }
    ...
    Serial.printf(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Connecting to ws://%s:%d%s...\n", host, port, path);
...
void WebSocketClient::begin(IPAddress ip, uint16_t port, const char* path) {
    ...
    Serial.printf(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Connection timeout: %lu ms\n", NetworkConfig::WS_CONNECTION_TIMEOUT_MS);
    Serial.printf(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Local IP: %s\n", WiFi.localIP().toString().c_str());
...
void WebSocketClient::update() {
    ...
    if (g_debugEnabled && (uint32_t)(millis() - s_lastLoopLog) >= 500) {  // Log every 500ms
        Serial.printf(COLOR_DEBUG "[DEBUG]" ANSI_RESET " WebSocketClient::update before loop - Heap: free=%u minFree=%u largest=%u status=%d\n",
    ...
    if (loopDurationMs > 1000) {
        Serial.printf(COLOR_WEBSOCKET "[WS]" ANSI_RESET " WARNING: _ws.loop() took %lu ms (status=%d)\n", 
    ...
    // #region agent log
#if ENABLE_WS_DIAGNOSTICS
    if (g_debugEnabled && (uint32_t)(millis() - s_lastLoopLog) >= 500) {
        Serial.printf(COLOR_DEBUG "[DEBUG]" ANSI_RESET " WebSocketClient::update after loop - Heap: free=%u minFree=%u largest=%u\n",
    ...
    if (_status == WebSocketStatus::CONNECTING) {
        if (s_connectingStartTime == 0) {
            ...
            Serial.println(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Connection timeout, resetting state");
            _status = WebSocketStatus::DISCONNECTED;
...
void WebSocketClient::disconnect() {
    Serial.println(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Disconnecting...");
    _shouldReconnect = false;
    _ws.disconnect();
    _status = WebSocketStatus::DISCONNECTED;
    _pendingHello = false;
...
void WebSocketClient::handleEvent(WStype_t type, uint8_t* payload, size_t length) {
    switch (type) {
        case WStype_DISCONNECTED:
            {
                ...
                Serial.printf(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Disconnected (reason: \"%s\", delay: %lu ms)\n",
        ...
        case WStype_CONNECTED:
            {
                Serial.println(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Connected to server");
                Serial.printf(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Server IP: %s:%d%s\n", 
        ...
        case WStype_TEXT:
            {
                ...
                if (_messageCallback) {
                    ...
                    if (!error) {
                        ...
                        Serial.printf(COLOR_WEBSOCKET "[WS]" ANSI_RESET " JSON parse error: %s\n", error.c_str());
                    }
                }
            }
            break;
        ...
        case WStype_ERROR:
            {
                Serial.printf(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Error occurred (delay: %lu ms)\n", _reconnectDelay);
                Serial.printf(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Target: ws://%s:%d%s\n",
                              _useIP ? _serverIP.toString().c_str() : (_serverHost ? _serverHost : "unknown"),
                              _serverPort, _serverPath);
                Serial.printf(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Local IP: %s, WiFi Status: %d\n", 
...
void WebSocketClient::attemptReconnect() {
    ...
    if (now - _lastReconnectAttempt >= _reconnectDelay) {
        ...
        Serial.printf(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Reconnecting (delay was: %lu ms)...\n", _reconnectDelay);
...
void WebSocketClient::sendJSON(const char* type, JsonDocument& doc) {
    if (!isConnected()) {
        ...
        Serial.printf(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Drop: not connected (type=%s, status=%d)\n", 
    ...
    if (!takeSendLock(SEND_MUTEX_TIMEOUT_MS)) {
        ...
        #ifdef ENABLE_VERBOSE_DEBUG
        Serial.printf(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Drop: send mutex busy (type=%s)\n", type ? type : "null");
        #endif
        _consecutiveSendFailures++;
        if (_consecutiveSendFailures > 3) {
            _sendDegraded = true;
            Serial.println(COLOR_WEBSOCKET "[WS]" ANSI_RESET " WARNING: Multiple send failures, marking as degraded");
        }
        return;
    }
    ...
    #ifdef ENABLE_VERBOSE_DEBUG
    if (g_debugEnabled) {
        Serial.printf(COLOR_DEBUG "[DEBUG]" ANSI_RESET " sendJSON before serialize - Heap: free=%u minFree=%u largest=%u type=%s\n",
    ...
    #ifdef ENABLE_VERBOSE_DEBUG
    if (g_debugEnabled) {
        Serial.printf(COLOR_DEBUG "[DEBUG]" ANSI_RESET " sendJSON after serialize - len=%u bufferSize=%u Heap: free=%u minFree=%u\n",
    ...
    if (len == 0 || len >= JSON_BUFFER_SIZE) {
        #ifdef ENABLE_VERBOSE_DEBUG
        Serial.printf(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Message too large, dropping (type=%s, len=%u, max=%u)\n",
    ...
    if (sendDuration > SEND_TIMEOUT_MS) {
        Serial.printf(COLOR_WEBSOCKET "[WS]" ANSI_RESET " WARNING: Send took %lu ms (threshold: %lu ms, type=%s)\n",
    ...
    if (sendResult) {
        _consecutiveSendFailures = 0;
        if (_sendDegraded) {
            Serial.println(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Send succeeded, clearing degraded state");
            _sendDegraded = false;
        }
    } else {
        _consecutiveSendFailures++;
        if (_consecutiveSendFailures > 3) {
            _sendDegraded = true;
            Serial.printf(COLOR_WEBSOCKET "[WS]" ANSI_RESET " WARNING: %u consecutive send failures, marking as degraded\n",
                          _consecutiveSendFailures);
        }
    }
    ...
    #ifdef ENABLE_VERBOSE_DEBUG
    if (g_debugEnabled) {
        Serial.printf(COLOR_DEBUG "[DEBUG]" ANSI_RESET " sendJSON after sendTXT - Heap: free=%u minFree=%u largest=%u sendResult=%d duration=%lu\n",
                      ESP.getFreeHeap(), ESP.getMinFreeHeap(), ESP.getMaxAllocHeap(), 
...
void WebSocketClient::sendHelloMessage() {
    ...
    Serial.println(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Sending hello (getStatus)");
    JsonDocument doc;
...
void WebSocketClient::requestZonesState() {
    if (!isConnected()) return;
    Serial.println(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Requesting zone state (zones.get)");
    JsonDocument doc;
...
void WebSocketClient::sendZoneEffect(uint8_t zoneId, uint8_t effectId) {
    ...
    if (!canSend(paramIndex)) {
        queueParameterChange(paramIndex, effectId, "zones.setEffect", zoneId);
        return;
    }
...
void WebSocketClient::sendZoneBrightness(uint8_t zoneId, uint8_t value) {
    ...
    if (!canSend(paramIndex)) {
        queueParameterChange(paramIndex, value, "zones.setBrightness", zoneId);
        return;
    }
...
void WebSocketClient::sendZoneSpeed(uint8_t zoneId, uint8_t value) {
    ...
    if (!canSend(paramIndex)) {
        queueParameterChange(paramIndex, value, "zones.setSpeed", zoneId);
        return;
    }
...
void WebSocketClient::sendZonePalette(uint8_t zoneId, uint8_t paletteId) {
    ...
    if (!canSend(paramIndex)) {
        queueParameterChange(paramIndex, paletteId, "zones.setPalette", zoneId);
        return;
    }
...
void WebSocketClient::sendZoneBlend(uint8_t zoneId, uint8_t blendMode) {
    ...
    if (!canSend(paramIndex)) {
        queueParameterChange(paramIndex, blendMode, "zones.setBlend", zoneId);
        return;
    }
...
void WebSocketClient::sendColorCorrectionConfig(bool gammaEnabled, float gammaValue,
    ...
    Serial.printf(COLOR_WEBSOCKET "[WS]" ANSI_RESET " Sending colorCorrection.setConfig: gamma=%s(%.1f) ae=%s brown=%s mode=%d\n",
...
void WebSocketClient::processSendQueue() {
    ...
    for (size_t i = 0; i < SEND_QUEUE_SIZE; i++) {
        ...
        if (takeSendLock(0)) {  // Try lock, don't wait
            ...
            uint32_t sendDuration = millis() - _sendAttemptStartTime;
            if (sendDuration > SEND_TIMEOUT_MS) {
                Serial.printf(COLOR_WEBSOCKET "[WS]" ANSI_RESET " WARNING: Send took %lu ms (threshold: %lu ms)\n", 
