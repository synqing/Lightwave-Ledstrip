...
public:
     ...
     */
    ParameterHandler(DualEncoderService* encoderService, WebSocketClient* wsClient = nullptr, HubState* hubState = nullptr);
    ...
    /**
     * Apply a value locally (cache + encoder hardware) without sending to the hub/network.
     * Used for preset recall and NVS restore so UI state matches the hardware.
     */
    void applyLocalValue(uint8_t index, uint16_t value, bool writeEncoder);
