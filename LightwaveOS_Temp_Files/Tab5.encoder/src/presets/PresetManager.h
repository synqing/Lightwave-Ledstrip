...
//   - Captures current state from HubState (authoritative) and ParameterHandler (UI sync)
//   - Stores presets via PresetStorage (NVS persistence)
//   - Applies presets via HubState setters (hub broadcasts via 50ms batching)
...
//
// Usage:
//   PresetManager presetMgr(&paramHandler, &hubState);
...
public:
     ...
     * @param hubState HubState for applying presets (authoritative)
     */
    PresetManager(ParameterHandler* paramHandler, HubState* hubState);
