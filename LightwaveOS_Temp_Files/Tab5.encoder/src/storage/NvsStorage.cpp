...
bool NvsStorage::init() {
    ...
    Serial.println(COLOR_NVS "[NVS]" ANSI_RESET " Initialising NVS flash...");
    ...
    if (err == ESP_ERR_NVS_NO_FREE_PAGES || err == ESP_ERR_NVS_NEW_VERSION_FOUND) {
        Serial.println(COLOR_NVS "[NVS]" ANSI_RESET " NVS partition needs erase - performing first-boot init");
        err = nvs_flash_erase();
        if (err != ESP_OK) {
            Serial.printf(COLOR_NVS "[NVS]" ANSI_RESET " ERROR: nvs_flash_erase failed: %s\n", esp_err_to_name(err));
            return false;
        }
        err = nvs_flash_init();
    }
    ...
    if (err != ESP_OK) {
        Serial.printf(COLOR_NVS "[NVS]" ANSI_RESET " ERROR: nvs_flash_init failed: %s\n", esp_err_to_name(err));
        return false;
    }
    ...
    if (err != ESP_OK) {
        Serial.printf(COLOR_NVS "[NVS]" ANSI_RESET " ERROR: nvs_open('%s') failed: %s\n", NVS_NAMESPACE, esp_err_to_name(err));
        return false;
    }

    s_initialized = true;
    Serial.printf(COLOR_NVS "[NVS]" ANSI_RESET " Initialized successfully (namespace: '%s')\n", NVS_NAMESPACE);
...
uint16_t NvsStorage::loadParameter(uint8_t index, uint16_t defaultValue) {
    ...
    if (err == ESP_ERR_NVS_NOT_FOUND) {
        ...
        Serial.printf(COLOR_NVS "[NVS]" ANSI_RESET " WARN: Failed to load %s: %s\n", key, esp_err_to_name(err));
        return defaultValue;
    }
...
uint8_t NvsStorage::loadAllParameters(uint16_t values[PARAM_COUNT]) {
    if (!s_initialized) {
        Serial.println(COLOR_NVS "[NVS]" ANSI_RESET " ERROR: loadAllParameters called before init");
        // Fill with defaults
        for (uint8_t i = 0; i < PARAM_COUNT; i++) {
    ...
    if (loadedCount == 0) {
        Serial.println(COLOR_NVS "[NVS]" ANSI_RESET " First boot - using default values");
    } else {
        Serial.printf(COLOR_NVS "[NVS]" ANSI_RESET " Loaded %d/%d parameters from flash\n", loadedCount, PARAM_COUNT);
    }
...
void NvsStorage::update() {
    ...
    if (didCommit) {
        ...
        if (err != ESP_OK) {
            Serial.printf(COLOR_NVS "[NVS]" ANSI_RESET " WARN: nvs_commit failed: %s\n", esp_err_to_name(err));
        }
    }
}
...
void NvsStorage::flushAll() {
    ...
    Serial.println(COLOR_NVS "[NVS]" ANSI_RESET " Flushing all pending saves...");
    ...
    if (err != ESP_OK) {
        ...
        Serial.println(COLOR_NVS "[NVS]" ANSI_RESET " Flush complete");
    }
}
...
bool NvsStorage::saveParameter(uint8_t index, uint16_t value) {
    ...
    if (err != ESP_OK) {
        Serial.printf(COLOR_NVS "[NVS]" ANSI_RESET " ERROR: Failed to save %s=%d: %s\n", key, value, esp_err_to_name(err));
        return false;
    }

    // Note: nvs_commit is called by update() after batch of saves
    Serial.printf(COLOR_NVS "[NVS]" ANSI_RESET " Saved %s=%d\n", key, value);
    return true;
}
...
uint8_t NvsStorage::saveAllParameters(const uint16_t values[PARAM_COUNT]) {
    if (!s_initialized) {
        Serial.println(COLOR_NVS "[NVS]" ANSI_RESET " ERROR: saveAllParameters called before init");
        return 0;
    }
    ...
    Serial.printf(COLOR_NVS "[NVS]" ANSI_RESET " Batch saved %d/%d parameters\n", savedCount, PARAM_COUNT);
...
bool NvsStorage::eraseAll() {
    if (!s_initialized) {
        Serial.println(COLOR_NVS "[NVS]" ANSI_RESET " ERROR: eraseAll called before init");
        return false;
    }

    Serial.println(COLOR_NVS "[NVS]" ANSI_RESET " Erasing all parameters...");

    // Erase all keys in our namespace
    esp_err_t err = nvs_erase_all(s_handle);
    if (err != ESP_OK) {
        Serial.printf(COLOR_NVS "[NVS]" ANSI_RESET " ERROR: nvs_erase_all failed: %s\n", esp_err_to_name(err));
        return false;
    }
    ...
    if (err != ESP_OK) {
        Serial.printf(COLOR_NVS "[NVS]" ANSI_RESET " ERROR: nvs_commit failed: %s\n", esp_err_to_name(err));
        return false;
    }
    ...
    Serial.println(COLOR_NVS "[NVS]" ANSI_RESET " All parameters erased");
    return true;
}
