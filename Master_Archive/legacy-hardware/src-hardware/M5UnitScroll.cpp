...
bool M5UnitScroll::begin(TwoWire *wire, uint8_t addr, uint8_t sda, uint8_t scl, uint32_t speed) {
    ...
    _scl   = scl;
    _speed = speed;
    // Don't call begin() if Wire is already initialized
    // This prevents the "Bus already started" warning
    // The main program should initialize Wire before calling this
