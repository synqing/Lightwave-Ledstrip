...
// Potentially other project-specific includes are already there or in a main header
// Assuming TAG, REFERENCE_FPS etc. might come from a config or other local header
...
void loop_gpu(void *pvParameters) {
	while (1) {
		...
		run_gpu();
		run_gpu();
		vTaskDelay(1); // Yield to allow other tasks on Core 1 to run
	}
}