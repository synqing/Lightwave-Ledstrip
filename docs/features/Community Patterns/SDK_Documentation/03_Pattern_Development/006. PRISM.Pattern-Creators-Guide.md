...
### Project constraints (LightwaveOS LGP builds)

If you are creating patterns intended for the LightwaveOS dual‑edge Light Guide Plate (LGP) mode, two rules are non‑negotiable:

- **Centre origin only**: all propagation expands from LEDs 79/80 or contracts to LEDs 79/80.
- **No rainbows**: do not implement full‑spectrum hue sweeps or rainbow cycling.

Some code examples in this guide use random hue selection or broad HSV motion to illustrate concepts. Treat those as educational scaffolding; when targeting LGP mode, use restrained palettes and tell the story with phase, interference, diffusion, and contrast instead of hue‑wheel motion.
