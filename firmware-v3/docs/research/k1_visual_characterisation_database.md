...
| Waveform Runtime Timing Pressure | `0x1302`, `0x1313` | unknown; Captain has not yet linked this runtime trait to a visual statement. | K1v2 serial baseline after `555f841e`: both Waveform-family effects show no LED output faults but run near/over the 120 FPS frame period with high accumulated drops. | `firmware-v3/docs/research/k1_waveform_hybrid_serial_evidence_2026-05-07.md` (`0x1313`: `fps=117-119`, `avg_us=8500-8548`; `0x1302`: `fps=117`, `avg_us=8515-8546`; both `show_skips=0`, no RMT/output faults). | Frame-budget margin is narrow: `FRAME_TIME_US=8333`, protected LED show averages about `6.18 ms`, and both Waveform effects add multiple full-strip passes plus shared VP post stages. Existing total-frame stats include LED show, so they must not be treated as effect-code-only timing. | `firmware-v3/docs/research/k1_waveform_hybrid_serial_evidence_2026-05-07.md` | K1v2 serial `vp stack`, `s`, `dbg memory`; trace capture attempt failed because the current image did not emit MabuTrace markers. | parked | Capture per-layer timing before optimisation; do not tune effect render loops or global VP stages from total-frame averages alone. |
