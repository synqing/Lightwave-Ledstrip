# K1 Visual Pipeline — Synergy Topology Extraction Protocol
...
This is a RESEARCH task, not a code modification task. No source code will be modified. All outputs are analytical documents. Parallel agent sandboxing (cp -r) is NOT required.

**Pass 1 — Parallelise via subagents.**

Use `/dispatching-parallel-agents` to spawn subagents for Pass 1. This is a multi-target, multi-document analytical decomposition — it MUST be delegated, not processed sequentially in main context.

Spawn 4 subagents with the following scopes:

| Subagent | Reads (primary) | Reads (secondary SSAs) | Returns |
|----------|-----------------|------------------------|---------|
| SA-1: Audio & Persistence | Taxonomy + Catalogue | SSA-AudioDriver, SSA-Persistence | Domain registry fragment + pairwise interactions for audio/temporal domains |
| SA-2: Physics & Geometry | Taxonomy + Catalogue | SSA-Physics, SSA-Geometry | Domain registry fragment + pairwise interactions for spatial/physical domains |
| SA-3: Composition & Cross-Lineage | Taxonomy + Catalogue | SSA-Composition, SSA-CrossLineage | Domain registry fragment + pairwise interactions for composition/lineage domains |
| SA-4: K1-Specific & Product Strategy | Taxonomy + Catalogue | SSA-K1Specific, SSA-ProductStrategy | Domain registry fragment + pairwise interactions for K1 affordances + viability filter |

Each subagent prompt MUST include (per CLAUDE.md Subagent Delegation Protocol):
- The K1 hard constraints (centre-origin 79/80, dual 2×160, 120 FPS, 2.0ms ceiling, no heap in render, dt-correct smoothing)
- The Pass 1 Instructions and Output Format from this document
- The instruction: *"Before starting work, assess: do you have sufficient context to complete this task correctly? If NOT, state what is missing rather than guessing."*
- The scope boundary: *"You are producing a DOMAIN REGISTRY FRAGMENT. Do not attempt the full interaction matrix — only evaluate interactions between domains YOU identify. The orchestrator will merge and cross-evaluate."*

Each subagent MUST return using the mandatory return contract format:
```
Files inspected: [count]
Domain registry fragment: [structured per Pass 1 format]
Pairwise interactions (within-scope only): [structured per Pass 1 format]
Divergent domains found (singleton SSA proposals): [list with [divergent] flag]
Confidence: [high/medium/low + reasoning]
Open questions: [if any]
```

Target: each subagent completes in under 30K tokens.

**Orchestrator (main context) — after all 4 subagents return:**
1. Merge domain registry fragments. Deduplicate. Resolve naming conflicts.
2. Build the FULL interaction matrix — including cross-subagent domain pairs that no individual subagent could evaluate.
3. Identify higher-order synergies (triadic+).
4. Write `PASS_1_DOMAIN_REGISTRY.md` to disk.
5. Proceed to Pass 2.

**Passes 2, 3, and 4 — execute sequentially in main context.**

These passes require cross-domain graph reasoning that cannot be meaningfully parallelised. The orchestrator executes each pass directly, reading the previous pass's output file as input.

**Context management for sequential passes:** After each pass output is written to disk, treat earlier source reads as evictable. Re-read specific sources on demand for later passes; do not rely on retained memory of files read more than one pass ago. If you find yourself uncertain about a source claim, re-read the source rather than reconstruct from memory.

If context becomes degraded during Passes 2–4 (you notice yourself losing track of the graph structure, or repeating analysis), use `/crispy:handoff` to rotate into a fresh session. Write the current pass output to disk BEFORE rotating so the next session can resume from the file.

### Dual-LLM Awareness

This same protocol is being run independently on GPT Codex. You do not need to account for this in your analysis — just produce the best output you can. The outputs will be compared afterward by Captain. Where the two runs converge, confidence is high. Where they diverge, the divergence is signal. Strong, defensible disagreement with consensus is more valuable than weak agreement. Where you would bet against the field, say so. Where you'd hedge, hedge — but flag the hedge explicitly.
...
**Secondary sources (SSA raw outputs):**
...
All outputs land in `firmware-v3/docs/research/synergy-topology/`:
