...
| Serial status label `Drops:` | `ActorSystem::printStatus()` printed `rs.frameDrops` beside `Frames`; the 2026-05-16 follow-up now prints `OverBudget:` with a percentage derived from `framesRendered`. | Hardware frame skips or RMT output drops. | Serial evidence can be misclassified as AP/VP coupling, render failure, or LED suppression. | `ActorSystem.cpp:842-853`; `ap_vp_contract_frame_drop_investigation_2026-05-14.md:74-81,231-240`. | Implemented for the serial-only surface. Keep `LED show ... skips=...` separate. |
...
3. DONE 2026-05-16: Treat protocol `cpuPercent` as migration-sensitive. The same field name appears on multiple REST/WS surfaces with different owners, so field descriptions now document owner and denominator; code changes should still wait for a client-compatibility plan.
...
| 2026-05-16 | codex:gpt-5.5 | Added REST/WS contract descriptions for renderer `cpuPercent` and audio benchmark `cpuLoadPercent`; no field rename or firmware behaviour change. |
