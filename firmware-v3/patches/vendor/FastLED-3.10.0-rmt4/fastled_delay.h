...
extern "C" {
#endif
/** Monotonic time in microseconds; safe when flash cache is suspended (unlike micros() when CONFIG_ARDUINO_ISR_IRAM is off). */
...
public:
	...
	void mark() {
#if defined(ESP32)
		// RMT ISR (FastLED idf4_rmt_impl doneOnChannel) calls mark(). micros() can be linked
		// into flash when CONFIG_ARDUINO_ISR_IRAM is unset, which panics if another core
		// suspends the flash cache during NVS/WiFi (Guru Meditation: cache disabled but cached region accessed).
		mLastMicros = (uint16_t)((uint64_t)esp_timer_get_time() & 0xFFFF);
#else
