...
void ESP32RMTController::showPixels()
{
    ...
    if (gNumStarted == gNumControllers)
    {
        ...
        // -- Non-blocking transmit model:
        //    return immediately after all eligible channels are started.
        //    Completion bookkeeping and semaphore give happen in doneOnChannel().
    }
}
...
void IRAM_ATTR ESP32RMTController::doneOnChannel(rmt_channel_t channel, void *arg)
{
    ...
    if (gNext < gNumControllers)
    {
        // Queue the next pending controller immediately onto this freed channel.
        ESP32RMTController::startNext(channel);
        return;
    }
    ...
    // All active controllers completed for this frame.
    gWait.mark();
    gNumStarted = 0;
    gNumDone = 0;
    gNext = 0;
