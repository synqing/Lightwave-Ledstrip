/**
 * @file LGPBeatPrismOnsetAdvectEffect.cpp
 * @brief Pressure Advection variant of LGP Beat Prism Onset.
 *
 * VARIANT 2 — Pressure Advection (0x1E02)
 *
 * WHAT CHANGED (background motion only):
 *   - Spring breathing replaced with a mostly-anchored crystal field.
 *   - The kick front locally displaces the sampling position via a
 *     Gaussian influence kernel: exp(-|d - frontPos| * 10) * kickPulse.
 *   - advectOffset = frontInfluence * 0.08 shifts sample position.
 *   - Background uses slow phase drift (~0.20) and spoke density ~7.0.
 *   - Field is calm between hits, visibly deformed during kick travel.
 *
...
#include "LGPBeatPrismOnsetAdvectEffect.h"
...
bool LGPBeatPrismOnsetAdvectEffect::init(plugins::EffectContext& ctx) {
    (void)ctx;
    m_phase = 0.0f;
    m_prism = 0.0f;
    m_kickPulse = 0.0f;
...
void LGPBeatPrismOnsetAdvectEffect::render(plugins::EffectContext& ctx) {
    ...
    // --- VARIANT: Slow phase drift for anchored field ---
    m_phase += 0.20f * dtVisual;
    ...
    for (uint16_t dist = 0; dist < HALF_LENGTH; ++dist) {
        ...
        // --- VARIANT: Kick front locally advects the sampling position ---
        const float frontInfluence = expf(-fabsf(d - frontPos) * 10.0f) * m_kickPulse;
        const float advectOffset = frontInfluence * 0.08f;
        const float sampleD = d - advectOffset;

        // Spoke field: density ~7.0, slow phase drift
        const float spokes = fabsf(sinf((sampleD * 7.0f - m_phase * 0.12f) * EX_PI));
        ...
        // Kick front (PRESERVED — uses original d, not sampleD)
...
void LGPBeatPrismOnsetAdvectEffect::cleanup() {}

const plugins::EffectMetadata& LGPBeatPrismOnsetAdvectEffect::getMetadata() const {
    static plugins::EffectMetadata meta{
        "LGP Beat Prism Onset Advect",
        "Pressure advection — kick front locally displaces crystal field",
