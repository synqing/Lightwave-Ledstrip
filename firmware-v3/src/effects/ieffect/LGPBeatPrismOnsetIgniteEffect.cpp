/**
 * @file LGPBeatPrismOnsetIgniteEffect.cpp
 * @brief Progressive Ignition variant of LGP Beat Prism Onset.
 *
 * VARIANT 4 — Progressive Ignition (0x1E04)
 *
 * WHAT CHANGED (background motion only):
 *   - Spring breathing replaced with one-way ignition from centre to edge.
 *   - m_bgActivation tracks energy level with asymmetric smoothing:
 *     rise tau=0.10 (fast ignition), fall tau=0.40 (slow relaxation).
 *   - activeRadius = 0.18 + 0.82 * activation — defines how far from
 *     centre the field is lit.
 *   - Smoothstep engagement envelope: clamp01((activeRadius - d) / 0.12),
 *     shaped as engage^2 * (3 - 2*engage).
 *   - Underlying spoke skeleton is stable (density ~7.6, slow phase drift).
 *   - backgroundField = floor + (rawField - floor) * engageShaped.
 *
...
#include "LGPBeatPrismOnsetIgniteEffect.h"
...
bool LGPBeatPrismOnsetIgniteEffect::init(plugins::EffectContext& ctx) {
    (void)ctx;
    m_phase = 0.0f;
    m_prism = 0.0f;
    m_kickPulse = 0.0f;
...
void LGPBeatPrismOnsetIgniteEffect::render(plugins::EffectContext& ctx) {
    ...
    // --- VARIANT: Asymmetric activation tracking ---
    // Energy target: combination of bass + prism for overall "loudness"
    const float bass = ctx.audio.available ? bands[0] : 0.0f;
    const float activationTarget = clamp01f(bass * 0.6f + m_prism * 0.5f + m_kickPulse * 0.4f);
    // Asymmetric smoothing: fast rise (tau=0.10), slow fall (tau=0.40)
    const float activationTau = (activationTarget > m_bgActivation) ? 0.10f : 0.40f;
    m_bgActivation = smoothTo(m_bgActivation, activationTarget, dtSignal, activationTau);

    // Active radius: always some centre glow (0.18), expands to full (1.0)
    const float activeRadius = 0.18f + 0.82f * m_bgActivation;

    // Slow phase drift for stable underlying skeleton
    m_phase += 0.25f * dtVisual;
    ...
    for (uint16_t dist = 0; dist < HALF_LENGTH; ++dist) {
        ...
        // --- VARIANT: Smoothstep engagement envelope from centre ---
        const float engageRaw = clamp01f((activeRadius - d) / 0.12f);
        const float engageShaped = engageRaw * engageRaw * (3.0f - 2.0f * engageRaw);

        // Stable spoke skeleton (density ~7.6, slow phase drift)
        const float spokes = fabsf(sinf((d * 7.6f - m_phase * 0.10f) * EX_PI));
        ...
        // Raw background field (same combination as base)
        const float rawField = (0.20f + 0.80f * spokes) * (0.25f + 0.75f * facets) *
                               (0.20f + 0.80f * refract);
        // Apply ignition envelope: floor + (rawField - floor) * engageShaped
        static constexpr float kFloor = 0.04f;
        const float backgroundField = kFloor + (rawField - kFloor) * engageShaped;
        ...
        const float intensity = clamp01f(backgroundField + front * 1.10f + shimmer);
...
void LGPBeatPrismOnsetIgniteEffect::cleanup() {}

const plugins::EffectMetadata& LGPBeatPrismOnsetIgniteEffect::getMetadata() const {
    static plugins::EffectMetadata meta{
        "LGP Beat Prism Onset Ignite",
        "Progressive ignition — energy activates outward from centre, slow relaxation",
