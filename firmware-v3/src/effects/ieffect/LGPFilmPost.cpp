...
void apply(plugins::EffectContext& ctx, float speedNorm, QualityPreset preset) {
    ...
    const bool lightweight = (preset == QualityPreset::LIGHTWEIGHT);
    ...
    for (int i = 0; i < N; i++) {
        ...
        uint8_t rt, gt, bt;
        if (lightweight) {
            // Lightweight mode: skip ACES and dither; use centre sample + gamma + EMA only.
            rt = s_gamma[s_src[i].r];
            gt = s_gamma[s_src[i].g];
            bt = s_gamma[s_src[i].b];
        } else {
            ...
            rt = (uint8_t)lroundf(rf * 255.0f);
            gt = (uint8_t)lroundf(gf * 255.0f);
            bt = (uint8_t)lroundf(bf * 255.0f);
