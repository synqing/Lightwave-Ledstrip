...
LGPTimeReversalMirrorEffect_Mod3::LGPTimeReversalMirrorEffect_Mod3()
    : m_ps(nullptr)
    , m_phaseTimer(0.0f)
    , m_isReverse(false)
    , m_frameInPhase(0)
...
void LGPTimeReversalMirrorEffect_Mod3::seedField() {
    if (!m_ps) return;
...
void LGPTimeReversalMirrorEffect_Mod3::beginForwardPhase(bool reseedField) {
    m_isReverse = false;
    m_phaseTimer = 0.0f;
    m_frameInPhase = 0;
    m_historyWrite = 0;
...
void LGPTimeReversalMirrorEffect_Mod3::beginReversePhase() {
    m_isReverse = true;
    m_phaseTimer = 0.0f;
    m_frameInPhase = 0;
...
uint16_t LGPTimeReversalMirrorEffect_Mod3::historySlotFromChrono(uint16_t chronoIndex) const {
...
void LGPTimeReversalMirrorEffect_Mod3::render(plugins::EffectContext& ctx) {
    ...
    for (uint16_t i = 1; i < kFieldSize - 1; i++) {
        float gL = m_ps->u_curr[i] - m_ps->u_curr[i - 1];
        float gR = m_ps->u_curr[i + 1] - m_ps->u_curr[i];
        ...
        float env = m_ridgeEnv[i];
        float a = (rawRidge > env) ? ridgeAttack : ridgeRelease;
        m_ridgeEnv[i] = env + (rawRidge - env) * a;
    }
    m_ridgeEnv[0] = m_ridgeEnv[1];
    m_ridgeEnv[kFieldSize - 1] = m_ridgeEnv[kFieldSize - 2];
    ...
    for (uint16_t i = 0; i < STRIP_LENGTH; i++) {
        ...
        float ridge = sampleFieldLinear(
            m_ridgeEnv, kFieldSize,
            (float)fi + 0.7f * sinf(m_storyTime * 0.27f + (float)dist * 0.022f));
        ...
        float ridge2 = sampleFieldLinear(
            m_ridgeEnv, kFieldSize,
            (float)fi + 5.6f + 0.9f * sinf(m_storyTime * 0.25f - (float)dist * 0.020f));
