...
#define LW_LOG_TAG "LedDriver_P4"
#include "utils/Log.h"

namespace lightwaveos {
namespace hal {

LedDriver_P4::LedDriver_P4() {
    resetStats();
    memset(m_strip1, 0, sizeof(m_strip1));
    memset(m_strip2, 0, sizeof(m_strip2));
}

bool LedDriver_P4::init(const LedStripConfig& config) {
    m_dual = false;
    m_config1 = config;
    ...
    // Reset watchdog before blocking clear() call
    esp_task_wdt_reset();
    ...
    m_initialized = true;
    LW_LOGI("FastLED RMT5 init: %u LEDs on GPIO %u", config.ledCount, chip::gpio::LED_STRIP1_DATA);
    return true;
}

bool LedDriver_P4::initDual(const LedStripConfig& config1, const LedStripConfig& config2) {
    m_dual = true;
    m_config1 = config1;
    m_config2 = config2;
    ...
    // Reset watchdog before blocking clear() call
    // FastLED.clear(true) can take several ms and might trigger watchdog
    // if called during RendererActor onStart()
    esp_task_wdt_reset();
    ...
    m_initialized = true;
    LW_LOGI("FastLED RMT5 init: 2x%u LEDs on GPIO %u/%u", config1.ledCount,
             chip::gpio::LED_STRIP1_DATA, chip::gpio::LED_STRIP2_DATA);
    return true;
}

void LedDriver_P4::deinit() {
#ifndef NATIVE_BUILD
    FastLED.clear(true);
#endif
    m_initialized = false;
...
CRGB* LedDriver_P4::getBuffer() {
    return m_strip1;
}

CRGB* LedDriver_P4::getBuffer(uint8_t stripIndex) {
    if (stripIndex == 0) return m_strip1;
    if (stripIndex == 1) return m_strip2;
    return nullptr;
}

uint16_t LedDriver_P4::getTotalLedCount() const {
    return m_totalLeds;
}

uint16_t LedDriver_P4::getLedCount(uint8_t stripIndex) const {
    return (stripIndex < 2) ? m_stripCounts[stripIndex] : 0;
}

void LedDriver_P4::show() {
#ifndef NATIVE_BUILD
...
void LedDriver_P4::setBrightness(uint8_t brightness) {
...
uint8_t LedDriver_P4::getBrightness() const {
    return m_brightness;
}

void LedDriver_P4::setMaxPower(uint8_t volts, uint16_t milliamps) {
#ifndef NATIVE_BUILD
    FastLED.setMaxPowerInVoltsAndMilliamps(volts, milliamps);
#endif
}

void LedDriver_P4::clear(bool showNow) {
    fill(CRGB::Black, showNow);
}

void LedDriver_P4::fill(CRGB color, bool showNow) {
...
void LedDriver_P4::setPixel(uint16_t index, CRGB color) {
