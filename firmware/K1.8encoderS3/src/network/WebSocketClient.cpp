...
void WebSocketClient::begin(const char* host, uint16_t port, const char* path) {
    ...
    // Configure connection timeout (links2004/WebSockets uses setReconnectInterval for both reconnect and initial timeout)
    // Set a longer interval to give server more time to respond to upgrade request
...
void WebSocketClient::handleEvent(WStype_t type, uint8_t* payload, size_t length) {
    switch (type) {
        case WStype_DISCONNECTED:
            // NOTE: On ESP32 with links2004/WebSocketsClient, the disconnect payload is often a
            // short reason string, not a numeric close code. Do not print payload[0].
            {
                char reason[64];
                reason[0] = '\0';
                if (payload && length > 0) {
            ...
            // #region agent log
            {
                char reasonPrefix[48];
                memset(reasonPrefix, 0, sizeof(reasonPrefix));
                if (payload && length > 0) {
                    const size_t n = (length < (sizeof(reasonPrefix) - 1)) ? length : (sizeof(reasonPrefix) - 1);
                    memcpy(reasonPrefix, payload, n);
                    reasonPrefix[n] = '\0';
                }
                char data[280];
                snprintf(
                    data, sizeof(data),
                    "{\"event\":\"DISCONNECTED\",\"type\":%u,\"len\":%u,\"status\":%u,\"reconnectDelayMs\":%lu,\"reasonPrefix\":\"%s\"}",
                    (unsigned)type,
                    (unsigned)length,
                    (unsigned)_status,
                    (unsigned long)_reconnectDelay,
                ...
                agent_dbg_log("HwsA", "src/network/WebSocketClient.cpp:handleEvent", "ws.event", data);
            }
            // #endregion
            _status = WebSocketStatus::DISCONNECTED;
        ...
        case WStype_ERROR:
            ...
            // #region agent log
            {
                ...
                snprintf(
                    data, sizeof(data),
                    "{\"event\":\"ERROR\",\"len\":%u,\"status\":%u,\"reconnectDelayMs\":%lu}",
                    (unsigned)length,
                    (unsigned)_status,
                    (unsigned long)_reconnectDelay
                );
                agent_dbg_log("HwsD", "src/network/WebSocketClient.cpp:handleEvent", "ws.event", data);
            }
            // #endregion
...
void WebSocketClient::sendJSON(const char* type, StaticJsonDocument<128>& doc) {
    ...
    // #region agent log
    {
        // Capture a small prefix of outbound JSON to validate protocol framing.
        char snippet[64];
        memset(snippet, 0, sizeof(snippet));
        strncpy(snippet, _jsonBuffer, sizeof(snippet) - 1);
        char data[260];
        snprintf(
            data, sizeof(data),
            "{\"type\":\"%s\",\"bytes\":%u,\"jsonPrefix\":\"%s\"}",
            (type ? type : ""),
            (unsigned)len,
            snippet
        );
        agent_dbg_log("HwsB", "src/network/WebSocketClient.cpp:sendJSON", "ws.tx", data);
    }
    // #endregion
