...
void ParameterHandler::onEncoderChanged(uint8_t index, uint16_t value, bool wasReset) {
    ...
    // Update display
    if (m_displayUI) {
        m_displayUI->update(index, clampedValue);
        m_displayUI->setHighlight(index);
    }
    ...
    for (uint8_t i = 0; i < getParameterCount(); i++) {
        ...
        if (doc.containsKey(param->statusField)) {
            ...
            if (m_values[i] != newValue) {
                ...
                if (m_encoderCtrl) {
                    EncoderController::Parameter encParam = static_cast<EncoderController::Parameter>(param->encoderIndex);
                    m_encoderCtrl->setValue(encParam, newValue, false); // false = don't trigger callback
                }

                // Update display
                if (m_displayUI) {
                    m_displayUI->update(i, newValue);
                }
...
void ParameterHandler::sendParameterChange(const ParameterDef* param, uint8_t value) {
    ...
    // This will be refactored when WebSocketClient gets sendCommand() method
    switch (param->id) {
