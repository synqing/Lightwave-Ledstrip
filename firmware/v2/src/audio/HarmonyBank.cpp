/**
 * @file HarmonyBank.cpp
 * @brief Implementation of harmony-focused Goertzel bank
 *
...
#include "K1_GoertzelTables_16k.h"  // For kHarmonyBins_16k_64
#include <cmath>
#include <algorithm>
...
namespace {
    ...
    // AGC configuration (moderate attack/release)
    constexpr float AGC_ATTACK_TIME = 0.050f;   // 50ms attack
    constexpr float AGC_RELEASE_TIME = 0.300f;  // 300ms release
    constexpr float AGC_TARGET_LEVEL = 0.7f;
    constexpr float AGC_MAX_GAIN = 2.0f;        // Allow mild boost

    // Noise floor configuration
    constexpr float NOISE_TIME_CONSTANT = 1.0f;  // 1 second

    // Chroma stability configuration
    constexpr uint8_t CHROMA_STABILITY_WINDOW = 8;  // 8 frames (~128ms @ 62.5 Hz)

    // Chroma extraction frequency range
    constexpr float CHROMA_FREQ_START = 110.0f;   // A2 (first bin in kHarmonyBins_16k_64)
    constexpr float CHROMA_FREQ_END = 1975.0f;    // ~B6 (last bin)
}
    ...
    , m_flux(0.0f)
{
    ...
    // Set chroma extractor frequency range
    m_chromaExtractor.setFrequencyRange(CHROMA_FREQ_START, CHROMA_FREQ_END);

    // Initialize ChromaStability (heap-allocated due to large state)
    m_stability = new ChromaStability(CHROMA_STABILITY_WINDOW);

    // Clear output buffers
    memset(m_magnitudes, 0, sizeof(m_magnitudes));
    memset(m_chroma, 0, sizeof(m_chroma));
}
...
void HarmonyBank::process(const AudioRingBuffer<float, 2048>& ringBuffer) {
    ...
    // Step 4: Apply AGC (moderate attack/release)
    // Find peak magnitude
    float peak = 0.0f;
    for (uint8_t i = 0; i < NUM_BINS; i++) {
    ...
    // Step 5: Compute spectral flux (harmonic change detection)
    m_flux = m_noveltyFlux.compute(m_magnitudes, m_noiseFloor);

    // Step 6: Extract 12-bin chroma
    m_chromaExtractor.extract(m_magnitudes, NUM_BINS, m_chroma);

    // Step 7: Update chroma stability
    if (m_stability) {
        m_stability->update(m_chroma);
    }
}
