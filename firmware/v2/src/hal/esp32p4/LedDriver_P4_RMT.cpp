...
IRAM_ATTR static size_t rmt_encode_led_strip(
    ...
    switch (led_encoder->state) {
        case 0:  // Encode RGB byte data
            ...
            // Intentional fall-through to state 1
            [[fallthrough]];
...
bool LedDriver_P4_RMT::initRmtChannel(uint8_t gpio,
    ...
    tx_config.trans_queue_depth = kRmtTransQueueDepth;
    tx_config.intr_priority = 0;
    tx_config.flags.invert_out = 0;
    tx_config.flags.with_dma = 0;  // No DMA (Emotiscope-style)
...
void LedDriver_P4_RMT::show() {
    ...
    // ========================================================================
    // Use bounded timeout (50ms) instead of portMAX_DELAY to prevent watchdog
    // At 120 FPS, frame time is ~8.3ms, so 50ms is generous but won't hang forever
    constexpr TickType_t kRmtWaitTimeout = pdMS_TO_TICKS(50);
    
    if (!m_firstFrame) {
        // Reset watchdog before potentially blocking wait
        esp_task_wdt_reset();
        
        esp_err_t waitA = rmt_tx_wait_all_done(m_txChanA, kRmtWaitTimeout);
        if (waitA == ESP_ERR_TIMEOUT) {
            LW_LOGW("RMT channel A wait timeout - resetting encoder");
            rmt_encoder_reset(m_encoderA);
        }
        
        if (m_dual && m_txChanB) {
            esp_err_t waitB = rmt_tx_wait_all_done(m_txChanB, kRmtWaitTimeout);
            if (waitB == ESP_ERR_TIMEOUT) {
                LW_LOGW("RMT channel B wait timeout - resetting encoder");
                rmt_encoder_reset(m_encoderB);
            }
        }
    }
    m_firstFrame = false;
