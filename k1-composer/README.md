...
The diagnostics panel is accessible from the **Diagnostics** tab in the top navigation bar of the composer dashboard. It loads the diagnostic view in an embedded iframe, so no separate page or server is required.
...
- `k1-composer.html`: main composer dashboard (monolithic HTML/JS/CSS)
...
- `diagnostic/index.html`: standalone diagnostic panel (embedded via iframe from composer)
- `diagnostic/DiagnosticDataAdapter.js`: data adapter for diagnostic telemetry
- `diagnostic/proto-combined.jsx`: React diagnostic component (JSX, transpiled by Babel)
...
- Coverage gate: `python ../firmware-v3/tools/effect_tunables/generate_manifest.py --validate --enforce-all-families` passes
...
- Static manifest: `../firmware-v3/docs/effects/tunable_manifest.json`
