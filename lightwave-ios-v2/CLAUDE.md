...
- **Debounce**: All slider → network calls debounced 150ms. Pending flags prevent WS echo-back.
- **British English** in comments and docs.

## Design Principles

1. **Show the output** — LED strip preview is the hero element, not settings panels.
2. **Feel the music** — BPM and beat events are first-class. UI pulses on beats.
3. **Invite exploration** — Effects and palettes are visual (cards, swatches), not text lists.
...
Play tab:
  ...
  PatternPill + PalettePill (full-width, tappable → sheet)
...
## Key Files (v1 reference → v2 target)

| v1 File | v2 File | Changes |
|---------|---------|---------|
| `Views/RootView.swift` | `Views/Tabs/PlayTab.swift` | ScrollView→TabView, hero LED, new spacing |
| `Views/StatusBarView.swift` | `Views/Shared/PersistentStatusBar.swift` | 26pt→44pt, connection + now-playing |
...
| — | `Views/Play/HeroLEDPreview.swift` | NEW: Canvas-based 320-LED preview |
| — | `Views/Play/TransitionCard.swift` | NEW: 12 transition types |
| — | `Views/Audio/BPMHeroCard.swift` | NEW: 48pt BPM with beat ring |
| — | `ViewModels/TransitionViewModel.swift` | NEW: transition state |
| — | `ViewModels/ColourCorrectionViewModel.swift` | NEW: gamma, AE, guardrail |
...
|----|------|---|-------|------|
| #27138 | 1:36 AM | 🟣 | Snapshot testing infrastructure implemented with SnapshotTesting framework and 30 component test cases | ~785 |
| #26984 | 12:55 AM | 🔵 | iOS project includes UI testing bundle configuration | ~323 |
...
| #27795 | 1:00 PM | 🟣 | iOS Project Configuration, Test Targets, and CI Workflow | ~728 |

### Feb 9, 2026

| ID | Time | T | Title | Read |
|----|------|---|-------|------|
| #28255 | 3:03 PM | ✅ | LightwaveOS iOS app deployed to physical iPhone device | ~270 |
