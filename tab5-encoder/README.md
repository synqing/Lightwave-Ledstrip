...
```bash
# Build only:
PATH="/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin" pio run -e tab5 -d tab5-encoder
...
pio device monitor -p /dev/cu.usbmodem101 -d tab5-encoder -b 115200
```
...
2. **`-d tab5-encoder`** - Tells PlatformIO where the project is WITHOUT changing directory.
