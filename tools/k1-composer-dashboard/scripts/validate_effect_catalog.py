...
def discover_expected_effect_ids(repo_root: Path) -> Set[str]:
    ...
    headers = sorted((repo_root / "firmware/v2/src/effects/ieffect").glob("*.h"))
...
def current_firmware_hash(repo_root: Path) -> str:
    ...
    headers = sorted((repo_root / "firmware/v2/src/effects/ieffect").glob("*.h"))
    cpps = sorted((repo_root / "firmware/v2/src/effects/ieffect").glob("*.cpp"))
    effect_ids = repo_root / "firmware/v2/src/config/effect_ids.h"
...
def main() -> int:
    ...
    catalog_dir = (
        ...
        if args.catalog_dir
        else repo_root / "tools/k1-composer-dashboard/src/code-catalog/generated"
    )
